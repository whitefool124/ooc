...
export function validateMap(map, { formal = false } = {}) {
  ...
  const blocking = new Set(["HeavyCover", "PermanentWall", "AetherObjective", "AetherCrystal", "LootChest", "TowerMechanism", "OverloadDevice", "WardGenerator"]);
...
function mapToCSharp(map) {
  ...
  const terrain = map.terrain.map(item => `new LevelTerrainPlacement(${item.x}, ${item.y}, LevelTerrainKind.${item.kind}, ${item.mechanismKind || 0})`).join(",\n                    ");
