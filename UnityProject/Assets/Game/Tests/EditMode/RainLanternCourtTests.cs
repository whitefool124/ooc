...
namespace OCC.Combat.Tests
{
    public sealed class RainLanternCourtTests
    {
        ...
        public void FrozenLayout_UsesExactCoordinatesAndCombatants()
        {
            ...
            Assert.That(level.Width, Is.EqualTo(12));
            Assert.That(level.Height, Is.EqualTo(9));
            Assert.That(level.HeroSpawn, Is.EqualTo(new GridPosition(1, 7)));
            ...
            Assert.That(level.EnemyPlacements.Select(value => value.Position),
                Is.EqualTo(new[] { new GridPosition(7, 6), new GridPosition(8, 1) }));
            Assert.That(level.Terrain.Count(value => value.Kind == LevelTerrainKind.Water), Is.EqualTo(5));
            Assert.That(level.Terrain.Count(value => value.Kind == LevelTerrainKind.LampVine), Is.EqualTo(10));
            Assert.That(level.Terrain.Count(value => value.Kind == LevelTerrainKind.LightCover), Is.EqualTo(3));
            Assert.That(level.Terrain.Count(value => value.Kind == LevelTerrainKind.HeavyCover), Is.EqualTo(13));
        ...
        public void Build_AttachesFormalRulesAndExactEnemyVitals()
        {
            ...
            Assert.That(state.Map.GetTile(new GridPosition(2, 6)).IsWater, Is.True);
            Assert.That(state.Map.GetTile(new GridPosition(4, 0)).IsLampVine, Is.True);
            Assert.That(state.Map.HasLineOfSight(new GridPosition(4, 2), new GridPosition(3, 2)), Is.True);
            Assert.That(state.Map.HasLineOfSight(new GridPosition(4, 2), new GridPosition(2, 2)), Is.False);
        }
        ...
        public void FirstMoveThroughWater_CostsThreeExtinguishesAndTriggersOriginTalent()
        {
            ...
            CombatResolver.Resolve(state, CombatCommand.Move(hero.Id, new GridPosition(2, 6)));

            Assert.That(hero.Position, Is.EqualTo(new GridPosition(2, 6)));
        ...
        public void BorrowedCover_IsSelfOnlyNonStackingAndBoostsExactlyNextMove()
        {
            ...
            CombatResolver.Resolve(state, CombatCommand.Move(hero.Id, new GridPosition(1, 6)));
        ...
        public void Pyromancer_BurnsConfiguredFirstVineWithoutUnitDamage()
        {
            ...
            Assert.That(command.Destination, Is.EqualTo(new GridPosition(5, 2)));
            ...
            Assert.That(followUp.Destination, Is.EqualTo(new GridPosition(8, 3)));
        }
        ...
        public void ThreeFrozenRoutes_HaveDeterministicLegalMovementSegments()
        {
            ...
            {
                Assert.That(route.Value.First(), Is.EqualTo(new GridPosition(1, 7)), route.Key);
                for (int index = 1; index < route.Value.Count; index++)
                {
                    var path = state.Map.FindLowestCostPath(route.Value[index - 1], route.Value[index], 5,
