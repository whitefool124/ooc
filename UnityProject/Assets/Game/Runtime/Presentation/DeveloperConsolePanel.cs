...
namespace OCC.Combat.Presentation
{
    ...
    public sealed class DeveloperConsolePanel : MonoBehaviour
    {
        ...
        private void DrawTrainingRange()
        {
            ...
            GUI.Label(new Rect(list.x + 18, list.y + 42, 384, 28), $"第 {session.CurrentPage + 1} 页　共 {session.PageCount} 页　{session.Abilities.Count} 项能力");
            int row = 0;
            ...
            Rect detail = new Rect(panel.x + 466, panel.y + 158, 438, 700); Box(detail, "当前术式与数值参数");
            ...
            Rect result = new Rect(panel.x + 924, panel.y + 158, 668, 700); Box(result, "目标预览与施放结果");
        ...
        private void DrawTrainingLoadout()
        {
            ...
            GUI.Label(new Rect(list.x + 18, list.y + 42, 484, 28), $"第 {session.CurrentPage + 1} 页　共 {session.PageCount} 页　总计 {session.Abilities.Count} 项");
            int row = 0;
            foreach (TrainingRangeAbilityEntry ability in session.AbilitiesOnCurrentPage())
            {
                bool selected = ability.Id == session.CurrentAbility.Id;
