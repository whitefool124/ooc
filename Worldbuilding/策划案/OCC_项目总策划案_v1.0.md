...
右侧信息栏参考坐标为（1456，80），尺寸448×768，底边与底部指令栏之间保留16px。行动值、英雄概况两区分别为（16，16，416，480）、（16，508，416，244），使用2px细框和24px正文；不新增第三块常驻战斗记录区。两区各有独立翻面按钮，当前战斗内分别保持自己的正反面状态；Tab 同时翻转两区，无论两区此前是否同步。英雄概况正面保留生命、护盾、个人魔力与状态；背面为“玩家状态／被动效果”两栏，左栏显示行动、资源、护盾与当前状态，右栏显示当前生效的被动及来源，无内容时显示明确空态。行动值正面显示0／100／199轴、全局进度和最多五名单位的速度、当前值及预览变化；背面按最新在前读取真实战斗事件记录，内容超出时可滚动查看。翻面采用不受时间缩放影响的水平收拢—中点换面—水平展开动画；动画期间锁住对应按钮，重复输入不得造成中间态或改变玩法状态。每次行动值变化时，数字与条形长度都从当前显示值平滑过渡；预览改变预计出手顺位时，只在对应行右侧循环显示绿色上箭头或红色下箭头，顺位不变时隐藏，不增加解释文字。当前操作、目标合法性和即时决策摘要迁移到战场底部提示条；完整原因通过悬停查看。行动点使用战场左下96×96菱形徽章显示真实当前值与本回合基础上限，不以固定三格表示。
...
战场底部决策提示条参考位置为（56，768），尺寸620×58；左侧为96×96行动点菱形徽章，提示条正文从徽章右缘之后开始。提示条使用24px FusionPixel12正常字形和26px行距：上行说明当前操作，下行显示目标与结算摘要，每行最多18个显示字符；不可执行原因用危险色，键盘选目标用冷青色，完整原因通过悬停读取。禁止自动第三行、缩小字号、假粗体和文字阴影；提示条不得遮挡底栏或右侧信息栏。
