...
| 背包 | 容量由已装备的背包决定；学院第一阶段当前投放的背包均为 6×10 格；物品按实际尺寸占格 |
...
- 卷轴通常使用 1 次后销毁。
- 法宝配置独立使用次数和启动方式。
- 背包容量由已装备的背包决定；学院第一阶段当前投放的背包均提供 6×10 格。
...
![装备与背包三列布局｜Pixso 最新稿（2026-09-10）](https://feishu.cn/file/KHxCbujM8oyMc7xTY9ycd7qgnJg)

**界面说明：**装备与背包页从左到右固定为三列：人物属性与状态详情（配合当前角色立绘）、装备与四格战术栏、六行十列背包格子。装备列内九个槽位按人体位置纵向组织：头部、项链、双手侧的武器与施法单元、胸部、双侧戒指与背包、鞋部；空位只承担人体对位，不显示占位卡。装备区与下方四格战术栏共同撑满中列，并保持统一对齐与间距。战术栏归入装备列，不再放在背包格子或独立详情列。术式栏继续作为 11B 独立分页；物品完整效果通过悬停、键盘／手柄聚焦或选中时的共用内容悬浮窗查看。

**交互规则：**可以拖动、旋转、装备、卸下、整理背包，并把合法战术物放入 1–4 战术栏。人物属性和持续状态随装备、道具及服务结果即时刷新；合法改动立即保存。有待领奖励时必须先腾出位置。切换到术式栏不改变未提交的拖拽状态。
...
![学院地图精简状态入口｜Pixso 最新稿（2026-09-10）](https://feishu.cn/file/T4tTbIz5bolHhFxlsi3c23fhn0U)
...
| 交付 | 固定效果 | 玩家操作与去向 |
|-|-|-|
| 轻装传令衣 | 蓝色胸部装备，2×3、负荷1；不提供回合盾，每个自己回合第一次移动行动的步数+1。 | 必须查看2×3占格并放入6×10背包；允许旋转。紧邻显示它与已装备夹棉练习衣“失去本单位回合开始时的2盾、获得首次移动+1”的差异。玩家可立即装备，也可保留在背包。 |
