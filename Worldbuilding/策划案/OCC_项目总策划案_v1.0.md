...
右侧信息栏参考坐标为（1456，80），尺寸448×768，底边与底部指令栏之间保留16px。英雄概况、行动值两区分别为（16，16，416，244）、（16，272，416，480），使用2px细框和24px正文；不新增第三块常驻战斗记录区。两区各有独立翻面按钮，当前战斗内分别保持自己的正反面状态；Tab 同时翻转两区，无论两区此前是否同步。英雄概况正面保留生命、护盾、个人魔力与状态；背面为“玩家状态／被动效果”两栏，左栏显示行动、资源、护盾与当前状态，右栏显示当前生效的被动及来源，无内容时显示明确空态。行动值正面显示0／100／199轴、全局进度和最多五名单位的速度、当前值及预览变化；背面按最新在前读取真实战斗事件记录，内容超出时可滚动查看。翻面采用不受时间缩放影响的水平收拢—中点换面—水平展开动画；动画期间锁住对应按钮，重复输入不得造成中间态或改变玩法状态。每次行动值变化时，数字与条形长度都从当前显示值平滑过渡；预览改变预计出手顺位时，只在对应行右侧循环显示绿色上箭头或红色下箭头，顺位不变时隐藏，不增加解释文字。当前操作、目标合法性和即时决策摘要迁移到战场底部提示条；完整原因通过悬停查看。行动点使用战场左下96×96菱形徽章显示真实当前值与本回合基础上限，不以固定三格表示。
...
当前行动者使用不分阵营的黑白轮次标志：角色主体下层显示外包64px格的四角框，参考外框72×72、黑色承托线宽6、白色上层线宽2、每角边长16；角色顶部居中显示16×14白色下指针并带黑色外轮廓。四角框和指针只占角色轮廓外缘，不遮挡角色主体、生命护盾、敌人意图或交互热点，也不接收输入。行动者变化时，整组标志沿屏幕空间从上一行动者先加速、后减速地移动到新行动者；首个行动者直接出现，行动者离开视口时隐藏。敌情徽记与角色主体、血盾状态区或其他徽记重叠时，在视口内就近避让，并用不接收输入的短正交引线指回所属格；空间不足时保留信息并选择遮挡最小的位置。
