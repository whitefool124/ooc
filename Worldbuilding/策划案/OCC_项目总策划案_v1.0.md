...
| P2 单轮创建 | 教程已完成且当前无活动单轮 | 写入模式、种子、版本、40个随机常规节点和该轮初始资源；不用于教程结束后的首轮续接 | 出发整备 |
...
教程阶段共11个完全预设节点：1个开始节点和10个内容节点。内容节点范围到第一场精英胜利后的商店首次展开，精英前为8个内容节点；开始节点不计入内容节点统计。完成第11个节点后，系统保留当前轮次状态，并按当前轮次种子生成、接入后续40个常规学院节点。教程阶段本身不使用随机生成，也不包含首领战。
...
- 库存、购买与完成状态读档不刷新。完成商店后保留当前角色、构筑、资源和教程记录；系统按当前轮次种子和版本内容池生成后续40个随机常规学院节点，与教程完成标记及继承状态一并保存，读档不重抽。
...
| P1 首轮固定教程段 | 首次学院存档创建成功 | 3 场普通战、3 个事件、工坊、医务室、第一场精英战、精英后商店及其奖励／构筑教学 | 保留全部状态并接入后续 40 个随机常规学院节点 | 第 6 章与教程数据表为正式规格 |
| P3 随机学院地图循环 | 首轮固定教程段完成，或后续单轮整备完成 | 在 6 个区域中选择普通战、精英战、事件、工坊、医务室和商店节点；积累资源并决定何时挑战首领 | 达到 28 学院时序强制进入首领，或完成 12 节点后主动提前挑战 | 内容池已有基础条目；逐节点完整内容包、制作状态与实机验收仍须按清单盘点 |
...
1920×1080参考画布，上部地图75%、右HUD25%；底栏1888×200、外边距16，五组宽度200/1100/152/176/204。战场默认64px参考格，提供128px放大档；全场默认768×576，位于顶栏下与底栏上安全区。新制战场地面、玩法地块、掩体与占格场景物使用64×64交付画布，在1920×1080下分别按1倍与2倍显示；单格透明物件的主要可读主体控制在约48×48内，原则上四边各留至少8px透明安全区，连接型地形覆盖和铺满地面的底图可按功能触边。现有32×32战场资产只作为过渡资源，不作为新制作基线。960×540使用与64px主稿同身份的独立32×32低分辨率伴随图，在两档分别按1倍与2倍显示，不直接缩放64px主稿。单位画布与脚底锚点规则不变，逻辑仍占一格；人物和意图按足底与空间分层。默认64px格的血条56×8、护盾56×4，足下分行；128px格按两倍显示。单位越格、相邻遮挡及藤内表现必须实机复核。

语义图16×16最多4色；地图节点类型图标使用原生48×48、最多12色，只表达开始、普通战、精英战、Boss、工坊、医务室、商店与事件的类型语义，当前、可达、完成与锁定状态由独立节点外框表达；材质拾取物24×24源稿以(4,4)居中到32×32；装备、法宝、术式图标与非战场单格资产保持32×32，相关多格背包素材保持(W×32)×(H×32)。新制战场地面、玩法地块、掩体与占格场景物使用64×64交付画布，透明单格物件以约48×48为主要可读主体预算，多格战场物件使用(W×64)×(H×64)；地形专属破坏或状态特效使用64×64单帧。以上64px战场资产都须另备独立适配的32×32低分辨率伴随图供960×540使用，不允许由运行时非整数缩放代替。单位64×64、人形高44–48、头12–14、脚底Y58、中心X32；通用VFX原生单帧32×32、最多12色、4–10独立帧；周边背景480×270、最多24色、2倍或4倍显示。各类详细色数、透明边与PPU按同源机器镜像。Point、Clamp、无MipMap、无压缩、源帧硬Alpha；屏幕绘制位置对齐物理像素。像素细节必须组成连续、有意图的块面，不以一像素散点或高频明暗抖动补材质。
