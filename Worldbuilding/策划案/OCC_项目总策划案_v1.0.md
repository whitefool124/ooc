...
每个新存档先在已开放角色中选择一名预设主角，再进行该角色的固定体验：从固定基础配置开始，到第一场精英战后的商店首次展开，或以精英战死亡结束。之后进入常驻学院轮次，内容池由版本配置决定。当前已完成并可选的角色只有维克多·维恩；其他角色的身份、基础配置与剧情入口必须在各自档案确认后才可开放。

> 选择角色 → 确认该角色的固定基础配置 → 进入首次地图 → 战斗或处理事件 → 领取奖励 → 工坊与医务室 → 第一场精英战 → 商店首次展开 → 常驻学院体验
...
![以太主界面｜Pixso 最新稿（2026-09-09）](https://feishu.cn/file/FFBgbH3tqoxpnYxkCf4cN9vxnVb)

**界面说明：**整个《以太》的主入口，以人生档案统摄学院、战争与转折三个阶段。当前版本只开放学院阶段，但主界面不得表现为学院专属入口；玩家可以开始新档、继续游戏、查看行程和打开设置。
...
![固定学生配置｜Pixso 最新稿（2026-09-09）](https://feishu.cn/file/QePPbouCDoAC0axeiQ9csGr5nyc)

**界面说明：**使用大立绘、角色简介、身份／专业、核心特性和底部角色队列完成预设角色选择；同页展示所选角色首次体验固定的出身、天赋和专属术式。当前维克多·维恩使用完整内容，其他尚未完成档案的角色槽只显示“未开放”，不得虚构姓名、能力或剧情。

**交互规则：**切换已开放角色时同步更新立绘、简介和固定配置；未开放角色槽不可确认。选择“以此角色开始”后创建存档并固定该档角色；返回不会创建存档。
