...
> 以太主界面 → 开始新游戏 → 角色选择与固定基础配置预览 → 确认角色并创建存档 → 学院介绍 → 单轮介绍与出发整备 → 进入教程地图 → 战斗或处理事件 → 领取奖励 → 工坊与医务室 → 第一场精英战 → 商店首次展开 → 生成并接续随机常规学院地图
...
| F01 | 确认角色 | F02 | 创建新存档、写入教程版本并固定该档角色，同时创建首轮教程状态；失败留在 F01 |
...
| 常规肉鸽 F21／F52 | 生命归零 | F61 | 记录失败战斗，不结算成功节点、学院时序或胜利奖励；教程战斗同样进入可重试结算 |
...
**交互规则：**从以太主界面的“开始新游戏”进入本页。切换已开放角色时同步更新立绘、简介和固定配置；未开放角色槽不可确认。选择“以此角色开始”后才创建存档、写入教程版本并固定该档角色，随后进入学院介绍；返回以太主界面不会创建存档或保留临时选择。
...
**结算与界面：**奖励页只把三项被动作为候选卡，固定装备、法宝与金币放在固定获得区；整包作为同一待领取事务处理。精英奖励不新增教学层。共用选择、去向与原子保存规则见第6.2.5节，精英胜败转换按第1.3节 F61 执行，商店与教程结束见第6.2.9节。
...
- **关键交互：**点击可购买商品即进入扣款与去向处理；容量不足转入 F41；离开商店完成教程。
