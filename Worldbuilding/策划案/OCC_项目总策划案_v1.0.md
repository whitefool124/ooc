...
| 背包 | 容量由已装备背包的列数与行数属性决定；背包列数上限为 10，行数按具体背包配置；物品按实际尺寸占格。尺寸必须显式写成“X 列×Y 行”，不再用方向不明的“X×Y”作为唯一口径 |
...
- 卷轴通常使用 1 次后销毁。
- 法宝配置独立使用次数和启动方式。
- 背包容量由已装备背包的列数与行数属性决定；列数上限为 10，行数按具体背包配置。学院第一阶段当前已登记的勘验背架与快挂整备架均为 6 列×10 行。
...
**界面说明：**装备与背包页从左到右固定为三列：人物属性与状态详情、装备与四格战术栏、背包。中列装备区使用 3×3 纯图形九槽，固定顺序为：武器、头部、施法单元；戒指Ⅰ、胸部、戒指Ⅱ；项链、背包、鞋部。空槽只显示栏位语义图，不使用文字占位卡；下方四个纯图形战术槽用槽号与剩余次数表达状态。右列背包网格按当前已装备背包的列数与行数属性生成，列数不超过 10；图中勘验背架示例为 6 列×10 行，共 60 格。多格物品按真实占格覆盖网格。术式栏继续作为 11B 独立分页；物品完整效果通过悬停、键盘／手柄聚焦或选中时的共用内容悬浮窗查看。稳定三列、九个装备槽与四个战术槽由可检查、可复用的页面骨架持有；背包网格按装备属性生成，运行时绑定真实角色、物品、次数、容量、已用格及瞬时交互反馈。

**交互规则：**可以拖动、用 R 键或右键旋转、装备、卸下、整理背包，并把合法战术物放入 1–4 战术栏。人物属性和持续状态随装备、道具及服务结果即时刷新；合法改动立即保存。有待领奖励时必须先腾出位置。切换到术式栏会取消未完成的瞬时拖拽，但不撤销此前已经保存的合法配置。
...
**界面说明：**在战斗中整理装备、材料和背包；网格按当前已装备背包的列数与行数属性显示，列数不超过 10。四个战术栏位嵌在背包工作区内，不另设战术栏面板或独立物品详情栏；筛选、排序、搜索、已用容量和多格物品都读取真实库存状态。

**交互规则：**支付费用后进入整理状态，可拖拽、旋转、筛选、搜索和换装，并可从背包直接关联四个战术栏位；不能锻造或专精术式。选中物品的完整信息按 Pixso 显示在页面详情区，悬停与焦点仍复用共用内容悬浮窗。费用见 3.4.2。
