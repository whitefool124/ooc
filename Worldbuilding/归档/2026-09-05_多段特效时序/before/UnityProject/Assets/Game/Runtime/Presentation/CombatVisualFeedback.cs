    ...
    public sealed class CombatVisualFeedback : MonoBehaviour, IResolvedCombatFeedbackSink
    {
        ...
        public void PlayOutcome(bool victory)
        {
            ...
            if (!AnimationsEnabled)
            {
                ...
                DOVirtual.DelayedCall(.9f, () => { if (card != null) Destroy(card); }).SetUpdate(true).SetTarget(card);
                return;
            }
        ...
        public void NotifyFireSpell(FireSpellDefinition spell, GridPosition source, IReadOnlyList<GridPosition> targetCells, bool isTriggered = false)
        {
            if (spell == null || targetCells == null || targetCells.Count == 0) return;
            UnitState sourceUnit = bootstrap.CurrentState?.Units.Values.FirstOrDefault(unit => unit.Position == source);
            GridPosition primary = targetCells[0];
            // Delayed effects already belong to a weapon hit, reaction or move. Recasting here
        ...
        private void PulseCell(GridPosition position, Color color, float duration)
        {
            ...
            DOTween.To(() => group.alpha, value => group.alpha = value, 0f, duration)
                .SetUpdate(true).SetTarget(pulse).OnComplete(() => Destroy(pulse));
        }
