...
public static class RenderReview
{
    ...
    public static string Run()
    {
        ...
        try
        {
            ...
            var equipment = bootstrap.CurrentState.RogueEquipment;
            var definitions = ArtifactCatalog.All.Where(a => ArtifactCatalog.IsCurrentlyUsable(a.Id)).Take(4).ToArray();
            for(int i = 0; i < definitions.Length; i++) {
                var item = equipment.CreateTacticalItem("review_item_" + i, definitions[i].Id, i, "static_review");
                if(!equipment.AddTacticalToBackpack(item) || !equipment.AssignQuickbar(i, item.InstanceId))
                    throw new InvalidOperationException("Cannot prepare occupied quickbar review.");
            }
