...
public static class RenderReview
{
    ...
    public static string Run()
    {
        ...
        try
        {
            ...
            var selection=(CombatSelectionController)Get(bootstrap,"selection");
            foreach(var action in new[]{"移动","攻击","技能1","技能6","搜刮","互动","最长中文样本"}){
                if(action=="最长中文样本") ((Text)Get(hud,"decisionLabel")).text=new string('界',15)+"\n"+new string('界',15);
                else { selection.SelectAction(action); Call(hud,"Refresh"); }
                Canvas.ForceUpdateCanvases();camera.Render();
                var label=(Text)Get(hud,"decisionLabel");var parent=(RectTransform)label.transform.parent;
                var points=label.canvasRenderer.GetMesh().vertices.Select(v=>parent.InverseTransformPoint(label.transform.TransformPoint(v))).ToArray();
                results.Add(action+": lines="+label.cachedTextGenerator.lineCount+" glyphX="+points.Min(v=>v.x)+":"+points.Max(v=>v.x)+" glyphY="+points.Min(v=>v.y)+":"+points.Max(v=>v.y)+" bottomMargin="+(points.Min(v=>v.y)-parent.rect.yMin)+" font="+label.font.name+" text="+label.text.Replace("\n","|"));
            }
            return string.Join("\n",results);
        }
        finally
