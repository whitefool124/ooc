...
启动后进入以太主界面。选择“开始新游戏”只进入角色选择，不创建存档；玩家在已开放角色中查看人物档案、核心特性与固定基础配置，确认“以此角色开始”后才创建存档并固定该档角色。选择“继续游戏”时不重新选择角色，而是按存档中的权威阻塞状态恢复。新存档随后进入该角色的固定体验：从学院介绍与固定基础配置开始，到第一场精英战后的商店首次展开，或以精英战死亡结束。之后进入常驻学院轮次，内容池由版本配置决定。当前已完成并可选的角色只有维克多·维恩；其他角色的身份、基础配置与剧情入口必须在各自档案确认后才可开放。

> 以太主界面 → 开始新游戏 → 角色选择与固定基础配置预览 → 确认角色并创建存档 → 学院介绍 → 单轮介绍与出发整备 → 进入首次地图 → 战斗或处理事件 → 领取奖励 → 工坊与医务室 → 第一场精英战 → 商店首次展开 → 常驻学院体验
...
| 阶段 | 进入条件 | 阶段职责 | 唯一正常出口 |
|-|-|-|-|
| P0 存档入口 | 启动完成 | 新建、继续、设置、存档异常处理；新建后选择已开放角色 | 固定引导或单轮创建 |
| P1 固定引导 | 新存档且未完成引导 | 角色选择、固定基础配置、短图、三战、事件、服务、精英与商店 | 标记引导完成并进入常规学院流程 |
...
| F01 | 确认角色 | F02 | 创建新存档、写入固定引导版本并固定该档角色；失败留在 F01 |
...
| 常规肉鸽 F21／F52 | 生命归零 | F61 | 记录失败战斗，不结算成功节点、学院时序或胜利奖励；首次固定体验同样进入可重试结算 |
...
- 首次固定体验的三场普通战和第一场精英战均允许放弃奖励。放弃前必须二次确认；确认后记录放弃、清除待领取状态并继续流程，固定奖励与候选奖励一并放弃。其他奖励是否允许放弃由对应节点说明。
...
战场由遭遇决定尺寸、边界、初始单位、目标、地形与交互物。首次固定体验的三场普通战和第一场精英战均为 12×9 格；其他战场使用各自的遭遇规格。
...
- 首次固定体验中，每件装备最多锻造一次，只能安装一个部件，安装后不能更换。
...
玩家只能在工坊选择术式和背包内的专精材料，确认前显示唯一结果。专精不随机，不重掷。首次固定体验中，每个术式只能选择一种专精，安装后不能更换。
...
| 脱线疾行 | 最大突进距离 3→4 格；行动力奖励不变 | 当前不开放 |
...
**界面说明：**使用大立绘、角色简介、身份／专业、核心特性和底部角色队列完成预设角色选择；同页预览所选角色首次体验固定的出身、天赋和专属术式。当前维克多·维恩使用完整内容，其他尚未完成档案的角色槽只显示“未开放”，不得虚构姓名、能力或剧情。

**交互规则：**从以太主界面的“开始新游戏”进入本页。切换已开放角色时同步更新立绘、简介和固定配置；未开放角色槽不可确认。选择“以此角色开始”后才创建存档、写入固定引导版本并固定该档角色，随后进入学院介绍；返回以太主界面不会创建存档或保留临时选择。
...
当前只有维克多·维恩完成并开放这套固定基础配置；首次体验不再提供独立的出身随机或二次选择。其他预设角色须在各自档案确认身份、天赋、专属术式和基础配置后，才可加入新存档角色选择。
...
| 固定候选 | 稀有度 | 效果与战斗2用途 |
|-|-|-|
| 焦痕跃进 | 少见 | 2 点行动点、4魔力、冷却2自身回合、每战无限；沿主轴直线突进最多3格，起点产生火场，并对路径邻接的敌方单位与可破坏物各造成8点武器伤害。火场的触发、伤害、持续回合与优先级在正式投放前按场地效果规则补齐。可越过灯藤增加的移动距离，第一回合进入中央藤圈。 |
...
增幅刻墨在本次固定体验中可强化已获得术式：焦痕跃进的路径邻接伤害8→10，起点火场不变；熔障爆点的中心伤害16→20，邻格8点火焰伤害不变；熔障校准的伤害8→10，标记奖励不变；脱线疾行的最大距离3→4格，行动力奖励不变；炉压震步的落地冲击伤害8→10，推位距离不变。工坊仍可在战斗3前后访问；首次装备锻造和首次术式专精都是精英入口条件，每项只结算一次且复访不重置。
...
- 库存、购买与完成状态读档不刷新。完成商店后保留当前角色、构筑、资源和教程记录；常规学院关卡只有在内容确认后才会开放，不生成临时占位关卡。
...
战斗配置在飞书在线表 OCC_战斗配置数据表_v1.0 中编辑；每张配置表都有对应的字段说明。制作与导表前先把飞书最新内容同步到本地 CSV。
...
火场持续时间尚未完成从旧行动条刻度到回合制的转换。F-T01、F-P-M03、F-P-U17、F-P-R11～F-P-R15、F-P-R20 及其关联内容保持停用；确认持续回合数并同步总案、数据表与实现后才能重新投放。
...
中文使用FusionPixel12正常字形，参考正文24px、标题48px、强反馈72px；无BestFit、假粗体、文字阴影或多重描边。玩家文案不用间隔点、竖线或斜线组织信息；短字段按标签对齐，长字段换行，数值比值写明当前、上限、剩余或总计，正式专名中的间隔点除外。术式卡268×76、4×2共8张，图标64×64、费用60×64；名称最多两行，使用2px 细框，不让文字侵入费用区或边框。九宫格厚框文字至少12px安全区，不足改细框或无框分隔。鼠标在同一地块连续停留2秒后，按固定顺序显示由多个独立悬浮窗组成的信息组：单位窗位于左侧，状态窗位于右上，地块窗位于右下；地块窗始终显示，单位窗与状态窗仅在地块上有单位时显示。切换地块、移出地块、打开操作菜单或移动视口立即关闭并重新计时。单位悬停同时可读脚下特殊地形；水面与灯藤不逐格常驻标名，当前操作范围显著，其他范围退出。
...
右侧信息栏参考坐标为（1456，80），尺寸448×768，底边与底部指令栏之间保留16px。英雄概况、行动值两区分别为（16，16，416，244）、（16，272，416，480），使用2px细框和24px正文；不设置战斗记录区。英雄概况保留生命、护盾、个人魔力与状态；行动值区显示0／100／199轴、全局进度和最多五名单位的速度、当前值及预览变化。当前操作、目标合法性和即时决策摘要迁移到战场底部提示条；完整原因通过悬停查看。行动点使用战场左下96×96菱形徽章显示真实当前值与本回合基础上限，不以固定三格表示。
