...
1. 检查配置并加载战斗，显示任务目标和敌人意图。
2. 行动条达到 100 的单位获得回合。
3. 主角回合开始时获得 3 行动点。
4. 选择动作和目标，查看预览，确认后结算。
5. 玩家结束回合后，敌人按公开意图行动，然后继续推进。
6. 目标达成、主角死亡或主动放弃时结束战斗。胜利进入战果与奖励，失败进入本轮结算。
...
1. 查看任务目标、敌人意图和剩余行动点。
2. 选择动作和目标，查看花费与结果。
3. 确认后结算；不能执行时说明原因，不扣资源。
4. 有行动点可继续行动，也可直接结束回合；结束后交给下一单位。
