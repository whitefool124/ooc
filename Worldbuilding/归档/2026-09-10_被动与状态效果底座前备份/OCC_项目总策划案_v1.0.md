...
- “刻”“刻度”和“行动条延时”不再作为技能、装备、法宝或状态的持续单位。旧内容中的“持续 N 刻”“延后 N 刻”和“轻延时”全部失效，不能按原数值直接换算成回合；行动提前与延后仍可作为明确修改当前行动条的独立效果。
...
- 术式只能在战斗外更换。
- 进入战斗后锁定当前配置。
- 术式显示[行动点](https://jcnlgassfnsm.feishu.cn/docx/Y1KydFPjMocJjYxMmFEcTblcnLf#doxcnotsNJZwSylkCxvyvE2c5Qd)、魔力、冷却、每战次数、选取范围、作用范围、目标和效果；不再显示已经废弃的“延时”或裸射程数字。
...
| 火弹 | 伤害 12→14 | 魔力 2→1 |
| 焦痕跃进 | 路径邻接伤害 8→10；起点火场采用自身配置，不受本增幅影响 | 待定 |
| 熔障爆点 | 中心伤害 16→20；邻格 8 点火焰伤害不变 | 待定 |
| 熔障校准 | 伤害 8→10；标记奖励不变 | 待定 |
| 脱线疾行 | 最大突进距离 3→4 格；行动力奖励不变 | 待定 |
| 炉压震步 | 落地冲击伤害 8→10；推位距离不变 | 待定 |
...
![装备与背包三列布局｜Pixso 最新稿（2026-09-10）](https://feishu.cn/file/BTJ9bjc8roasmQxkc8ZcRWBunZ1)

**界面说明：**装备与背包页从左到右固定为三列：人物属性与状态详情（配合当前角色立绘）、装备与四格战术栏、六行十列背包格子。战术栏归入装备列，不再放在背包格子或独立详情列。术式栏继续作为 11B 独立分页；物品完整效果通过悬停、键盘／手柄聚焦或选中时的共用内容悬浮窗查看。
...
![学院地图精简状态入口｜Pixso 最新稿（2026-09-10）](https://feishu.cn/file/NwIkbo56eoacz9x76aocc2Yvn0e)
...
当前只有维克多·维恩完成并开放这套固定基础配置；首次体验不再提供独立的出身随机或二次选择。其他预设角色须在各自档案确认身份、天赋、专属术式和基础配置后，才可加入新存档角色选择；后期转化待定。
...
| 借障导流 | 主动术式 | 1 AP、1魔力、冷却0、每战无限；仅在自身正交邻接轻／重掩体时可施放，获得4普通护盾，并使本回合下一次移动的最大步数加2；同回合不叠加。 |
| 灼触 | 主动术式 | 1 AP、1魔力、冷却0、每战无限；对正交邻接1格内一个可见可受击目标造成8点火焰伤害。近战，不穿墙或阻挡物。 |
| 火花 | 主动术式 | 1 AP、2魔力、冷却0、每战无限；对菱形4格内一个可见可受击目标造成6点火焰伤害。投射，不穿墙或阻挡物，可被轨迹上第一个单位拦截。 |
| 以太护幕 | 主动术式 | 1 AP、2魔力、冷却1自身回合、每战无限；范围为自身，获得6普通护盾。 |
| 回路调息 | 主动术式 | 1 AP、0魔力、冷却1自身回合、每战无限；范围为自身，恢复2个人魔力，不超过当前上限。 |
...
| 战斗2 | 灯藤＋蓄能晶簇＋中央藤圈宝箱；奖励为熔障校准／脱线疾行／炉压震步三选一。 |
...
| 固定候选 | 稀有度 | 效果与战斗2用途 |
|-|-|-|
| 焦痕跃进 | 少见 | 2 AP、4魔力、冷却2自身回合、每战无限；沿主轴直线突进最多3格，起点产生火场，并对路径邻接的敌方单位与可破坏物各造成8点武器伤害。火场的触发、伤害、持续回合与优先级在正式投放前按场地效果规则补齐。可越过灯藤增加的移动距离，第一回合进入中央藤圈。 |
| 熔障爆点 | 少见 | 2 AP、4魔力、冷却2自身回合、每战无限；选择菱形4格内一个可见可受击目标，对目标造成16点火焰伤害，并对其正交邻接1格内所有可受击目标各造成8点火焰伤害，可伤友军。中心与邻格均不区分单位或物件。用于等待敌人贴晶后引爆环境。 |
...
| 交付或选择 | 固定内容 | 玩家操作与去向 |
|-|-|-|
| 导位罗盘 | 1 AP、完整3次；选择菱形4格内一个可见友军或敌人，目标必须能沿四向主轴建立通向使用者的逐格路径；将目标拉近最多2格，逐格结算，遇边界、阻挡或占用格立即停止，不造成伤害。 | 固定获得并放入一个战术栏格；完整显示剩余次数。战斗2可用于调整敌人与晶簇的相对位置，但不是唯一解。 |
...
> **当前实施进度（2026-09-09）：** 新的第二组候选、统一伤害、摧毁回流、脱险提速、突进冲击及对应增幅专精已同步到运行时与数据表；事件3、奖励发放、存档往返和工坊专精已完成整套 EditMode 回归。以太主界面到角色固定配置页已通过正常 Play Mode 入口与视觉检查；三项术式在正式战斗中的连续操作节奏仍待专项 Play Mode 实测。

| 方向 | 奖励2 | 固定效果 |
|-|-|-|
| 摧毁回流 | 熔障校准 | 1 AP、2魔力、冷却2自身回合、每战无限；选择菱形4格内一个可见可受击目标，先施加持续4回合的熔障标记，再造成8点火焰伤害。标记不叠加，再次施加只刷新；被标记目标在标记期间被摧毁时，施法者恢复2个人魔力并获得4普通护盾。 |
| 脱险提速 | 脱线疾行 | 1 AP、3魔力、冷却2自身回合、每战无限；沿四向主轴直线突进至最多3格外的合法落点。若起点位于至少一个敌方公开攻击范围内，且终点离开施放前全部敌方公开攻击范围，则下一次自身回合行动力加一；不叠加。本技能不产生攻击。 |
| 突进冲击 | 炉压震步 | 1 AP、3魔力、冷却2自身回合、每战无限；沿四向主轴直线突进至最多2格外的合法空格。抵达后对落点正交邻接1格内所有可受击目标造成8点伤害，可伤友军，再将其中所有单位沿远离落点的方向推开1格；推位终点非法时单位留在原地。 |

玩家卡片的费用区显示行动点、魔力和冷却，范围作为固定详情字段；效果正文不重复费用数字。三项均为每战无限，因此不在卡片上显示使用次数。

| 术式 | 费用区 | 范围 | 玩家效果描述 |
|-|-|-|-|
| 熔障校准 | 1 AP／2魔力／冷却2 | 选取：菱形范围4格；作用：单点 | 对一个目标施加持续4回合的熔障标记，并造成8点伤害。被标记的目标在标记期间被摧毁时，你恢复2魔力并获得4护盾。 |
| 脱线疾行 | 1 AP／3魔力／冷却2 | 选取：直线范围3格；作用：沿路径 | 沿直线突进最多3格。如果这次突进躲开了全部攻击范围，下回合行动力加一。 |
| 炉压震步 | 1 AP／3魔力／冷却2 | 选取：直线范围2格；作用：落点相邻1格 | 沿直线突进最多2格。抵达时，对相邻的所有目标造成8点伤害，并将其中的单位推开1格。 |
...
事件3“折光庭校验签领”固定交付增幅刻墨×1，并在学院贡献+1与截击铃×1之间选择；截击铃为2 AP、完整2次，选择菱形3格内一个可见空格进行标记，持续至使用者下一次自身回合开始；首个进入的敌人受到12点伤害并沿进入方向被推离1格，随后消耗标记，推位遇阻只结算伤害。确认结果后开放战斗3。唯一合法去向自动完成；只有战术栏、背包或装备替换发生冲突时才进入去向处理。
...
> **当前实施进度（2026-09-09）：** 本关地图、敌人编成、固定坐标、器材匣、学院储能芯、浅水、状态教学、16 耐久封门晶簇、第二组技能、统一伤害与三条构筑反馈均已同步运行时和数据表，并通过整套 EditMode 回归；本关正式战斗中的操作、预览与表现仍待专项 Play Mode 实测。
...
| 器材匣 | H5 | 固定装有学院储能芯；个人魔力上限加2，装备或卸下不会免费增加当前魔力，卸下后超出新上限的部分立即截断。搜刮1 AP，拿取不重复收费。H4、H6、I5为永久重物块，唯一入口为G5。 |
...
> **当前实施进度（2026-09-09）：** 第三组发放、精英地图、公开锁线、浅水、灯藤、干地卸压、精英奖励事务、商店解锁、刻印战锤、24 耐久稳压晶簇、8 点贯场冲压、回火导流与统一伤害均已同步运行时和数据表，并通过整套 EditMode 回归；精英战到商店的完整手动流程、存档、预览与表现仍待专项 Play Mode 实测。

| 方向 | 奖励3 | 精英前完成形态 |
|-|-|-|
| 摧毁回流 | 刻印战锤 | 熔障爆点＋熔障校准＋增幅专精＋刻印战锤；重击2 AP，对相邻一个可受击目标造成16点物理伤害，单位与物件数值相同，可直接摧毁标准物件。 |
...
| 候选 | 类型 | 固定效果 |
|-|-|-|
| 刻印战锤 | 双手武器 | 重击2 AP；对正交邻接1格内一个可见可受击目标造成16点物理伤害。近战，不穿墙或阻挡物；不再追加物件专属伤害。旧“轻延时+4”失效，替代代价待定。 |
| 接触耦合环 | 施法单元 | 每个自己回合第一次施放M线术式时魔力成本减1，最低1。 |
| 余烬格挡 | 主动术式 | 1 AP、3魔力、冷却2自身回合、每战无限；立即获得12普通护盾，本回合下一次近战命中后再获得4普通护盾。 |
...
| 商品 | 价格 | 固定效果 |
|-|-|-|
| 解构楔 | 3金币 | 1 AP、完整4次；对正交邻接1格内一个轻／重掩体或装置造成8点伤害。目标权限仍限定为物件，但沿用统一伤害值。 |
| 移相线轴 | 6金币 | 1 AP、完整2次；选择菱形3格内一个可见、未占用、可站立空格，使用者以移相方式直接到达目标；不检查中间格，但不能跨越重掩体闭合边界，束缚时非法。旧“下一次行动延后4刻”失效。 |
