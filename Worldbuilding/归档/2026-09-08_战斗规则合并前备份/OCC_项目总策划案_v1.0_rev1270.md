...
- 步数是一次移动行动最多可走的格数。基础值为 5 格；迟缓时为 3 格；实际范围受可通行路径、地形、单位占位和阻挡影响。
- 一次移动行动只消耗 1 点[行动点](https://jcnlgassfnsm.feishu.cn/docx/Y1KydFPjMocJjYxMmFEcTblcnLf#doxcnotsNJZwSylkCxvyvE2c5Qd)，可移动格数受[步数](https://jcnlgassfnsm.feishu.cn/docx/Y1KydFPjMocJjYxMmFEcTblcnLf#doxcnPG7pIeIZWtLBO8Rikv69mb)限制。
...
- 每个单位拥有一条行动条，基础范围为 0～100。
- 战斗时间推进时，单位每经过一个时间单位获得等同于其有效速度的行动条。
- 行动条达到 100 时，单位进入待行动状态并获得一个回合。
- 单位行动期间，战斗时间暂停，其他单位的行动条不会继续增长。
- 单位结束回合后扣除 100 点行动条，超出 100 的部分保留，作为下一次行动的起始进度。
- 行动条最多为 199，即最多储存一个回合和下一回合 99 点进度，不能连续储存多个完整回合。
- 多个单位同时进入待行动状态时，当前行动条较高者先行动；行动条相同时，有效速度较高者先行动；仍相同时，按战斗开始时确定的固定顺序行动。
- 加速和减速通过改变有效速度，影响行动条的增长速度。
- 行动提前效果直接增加当前行动条；行动延后效果直接减少当前行动条，行动条最低不会低于 0。
...
- 结束回合时清空剩余[行动点](https://jcnlgassfnsm.feishu.cn/docx/Y1KydFPjMocJjYxMmFEcTblcnLf#doxcnotsNJZwSylkCxvyvE2c5Qd)，再扣除 100 点行动条并保留溢出进度。
- 死亡或退出战斗的单位立即移出行动条。任务目标达成后立即结束战斗，不等待其他待行动单位。

#### 玩家说明

速度决定行动条的充能速度。行动条达到 100 时，单位获得回合；速度越高，获得回合越频繁。超过 100 的进度不会浪费，而会保留到下一次充能。

#### 示例

一个速度 10 的单位每个时间单位获得 10 点行动条，需要 10 个时间单位充满。一个速度 5 的单位每次获得 5 点，需要 20 个时间单位充满。因此在相同时间内，速度 10 的单位通常可以行动两次。若某单位以 125 点行动条进入回合，行动结束后扣除 100 点，并保留 25 点作为下一轮的起始进度。
...
| 护盾 | 只在当前战斗内存在，按伤害结算顺序先于生命承受伤害；生命周期与回合刷新见 3.2.4。 |
| 行动点 | 单位在当前自身回合内支付动作费用的资源。获得回合时重置为 3，结束回合时清空；不能跨回合储存。 |
| 行动条 | 当前战斗中的行动进度，范围为 0～199；达到 100 时进入待行动状态。增长、溢出、排序和移除规则见 3.1.6。 |
...
- 所有动作都遵循“预览→合法性检查→确认→扣费→执行→结算→胜负检查→刷新”。检查失败时不扣费、不改变状态；一次命令不得只结算一部分。
...
- 主角移动一次消耗 1 点行动点；攻击、术式、道具和互动按各自配置消耗。
...
- 单位已经获得当前回合后，行动延后不会中断或撤销这个回合，只影响回合结束后的剩余进度；结束回合仍按 3.1.6 扣除 100，结果最低为 0。
...
覆盖存档、主动放弃本轮、保存并返回标题、丢弃高价值物品，以及配置允许时放弃奖励都必须弹出确认框。
...
![学院地图｜最新 Pixso 界面稿（2026-09-08）](https://feishu.cn/file/CSuJbTnl6o217sxWxOwcDinDn5F)
...
![战斗 HUD｜最新 Pixso 界面稿（2026-09-08）](https://feishu.cn/file/TIZ8bQVXIoo4vlxxwyockPqWnHe)

![战斗叠层、右键菜单与悬停说明｜最新 Pixso 界面稿（2026-09-08）](https://feishu.cn/file/XARjbP3GEolNbmxZhDWcy6STn8c)
...
- 底部中部：1–8 号术式。
- 底部右侧：互动、战术栏、结束回合。
- 顶部：背包、不可重开提示和离开；离开可保存返回标题或主动放弃本轮。
- 指令不可用时保留按钮，并显示原因。
...
结束回合和地图复位快捷键待定；肉鸽模式不设置重开战斗快捷键。

### 5.1.7 Pixso 全量界面清单与交互规则（2026-09-08）

<callout emoji="📐">
本节覆盖当前 Pixso 02—23 的全部 24 个页面／状态稿。飞书本文档是玩法、页面内容、交互结果和数据口径的最终基准；Pixso 用于表达布局、层级与状态，不得以示例内容反向改写本文档。候选内容尚未冻结时，稿内必须明确标注“待冻结”。
</callout>

#### 入口、首次说明与单轮创建

| 稿号／页面 | 页面与显示职责 | 主要交互 | 出口、保存与阻塞 |
|-|-|-|-|
| 02  <br/>首启设置 | 设备首次启动的一次性声音与画面配置；显示音量、分辨率和步骤进度。 | 调整数值、恢复默认值、继续；设置项在常驻“辅助设置”中仍可修改。 | 确认后写入本机设置并进入世界观介绍；不创建或修改玩法存档。 |
| 03  <br/>世界观介绍 | 游戏级首次播放的四页世界观说明；建立“以太可测量、可维护、有代价”的核心认知。 | 下一页、上一页（非首页）、跳过介绍；分页只影响阅读位置。 | 读完或跳过后写入游戏级已读标记并进入学院入口；不按存档重复播放。 |
| 04  <br/>学院入口 | F00 存档入口；显示开始首次体验、继续游戏、行程与行囊、辅助设置及有效存档状态。 | 新建进入覆盖确认或固定首轮；继续按权威阻塞状态恢复；其余按钮打开档案或设置。 | 无有效存档时“继续”禁用。新建覆盖须经 19 号确认框；继续时不得绕过活动战斗、待领奖励或保存失败。 |
| 05  <br/>固定出身三件套 | F01／P1 固定首轮入口；同页显示护障检修户、天赋“就地接线”和专属术式“借障导流”。 | 查看三件套完整说明、返回入口、确认并建立档案；首次体验不提供其他出身选择。 | 确认后创建固定首轮并保存；返回不创建存档，已存在存档时必须先完成覆盖确认。 |
| 06  <br/>学院介绍 | 每个新存档首次进入时展示 6 个学院区域、节点状态和地图循环规则。 | 阅读后进入学院地图；已读内容可从 20 号档案总览重看。 | 写入存档级已读标记；不能替代地图节点选择，也不消耗学院时序。 |
| 07  <br/>单轮介绍 | F03 创建单轮后的目标说明；显示肉鸽模式、学院时序 0／21／25／28 的提示区间和首领阶段规则。 | 开始本轮；选择“稍后在档案重看”只关闭说明，不改变单轮内容。 | 单轮种子与 40 节点内容在进入本页前已写入；离开说明进入 F04 出发整备，不能借关闭页面重抽。 |

#### 地图、事件、整备与功能节点

| 稿号／页面 | 页面与显示职责 | 主要交互 | 出口、保存与阻塞 |
|-|-|-|-|
| 08  <br/>学院地图 | F10 主循环；显示 40 个节点、6 个区域、连线、当前位置、可达／不可达／已完成状态、资源与学院时序。 | 单击节点只更新右下摘要；支持缩放、复位和关闭地图；已完成节点可回访。 | 浏览不扣费。只有在 09 号页确认前往后才更新当前位置并保存；回访不重复耗时或发奖。 |
| 09  <br/>节点摘要与出发准备 | F11／F20；显示节点类型、危险度、学院时序成本、目标、敌人、地形和奖励类型。 | 确认前往、确认出发、先不挑战；战前允许的配置修改必须明确展示。 | 确认节点先保存位置；确认出发创建战斗入口快照。先不挑战返回地图但角色仍停留于该节点，重进不重抽。 |
| 10  <br/>事件节点 | F30；显示叙事、当前资源、每个选项的条件、成本、确定结果和学院时序变化。 | 先选择再确认；无力支付时保留至少一个不产生负余额的合法结束选项；未确认可返回。 | 确认后原子结算资源、物品去向、节点完成与保存。回访只显示已选结果，不刷新选项或奖励。 |
| 11A  <br/>装备与背包 | F04／F41 的学院整备页；显示装备槽、6×10 背包、占格、重量／负荷、对象详情和去向阻塞。 | 选择、拖拽、旋转、装备、卸下、整理与确认丢弃；高价值丢弃使用 19 号确认框。 | 合法配置变更后保存。F41 有待领奖励时必须先解决去向，不能通过返回地图清除奖励。 |
| 11B  <br/>术式与战术栏 | 术式栏 1—8 与战术栏 1—4 的编组状态；详情显示费用、目标、效果、来源和不可用原因。 | 选择术式或战术道具后指定槽位；允许替换、移除与查看完整说明；只允许合法类型进入对应栏位。 | 离开前校验并保存构筑。战斗中改动须先通过 23 号页支付 1 行动点；学院整备不支付行动点。 |
| 11C  <br/>工坊 | F31；显示装备锻造、术式专精、合法材料、首次完成状态、唯一结果和精英门槛进度。 | 锻造按“装备→部件位→材料→确认安装”；专精按“术式→分支→材料→确认安装”。结果直接在当前加工卡预览。 | 每次加工独立保存，首轮各免费 1 次但消耗对应材料，安装后不可更换。可退出；不足或非法时按钮禁用并显示单一原因，复访不刷新。 |
| 12  <br/>医务室 | F32；显示当前／上限生命、免费健康确认、治疗和固定三种餐食，明确生命不会自然恢复。 | 免费确认只记录状态；治疗支付 1 学院贡献并恢复最大生命 50%；餐食最多购买一种，持续 3 场战斗。 | 健康确认、治疗和餐食分别保存，治疗与餐食次数独立。余额不足保留项目并说明原因；可退出，复访不刷新。 |
| 13  <br/>商店 | F33；首轮固定展示解构楔 3 金币、移相线轴 6 金币、测距护目镜 4 金币及售罄状态。 | 打开商品详情、比较、购买、打开行囊、保留货币或离开；每件商品各限购一次。 | 每次购买独立保存，库存、价格和售罄不因离开或读档刷新。空间不足转 F41，不能自动丢弃。 |

#### 战斗、奖励与结算

| 稿号／页面 | 页面与显示职责 | 主要交互 | 出口、保存与阻塞 |
|-|-|-|-|
| 14  <br/>战斗 HUD | F21 主界面；左侧战场约 75%，右侧 HUD 约 25%，底部为移动、武器、1—8 术式、互动、4 格战术栏和结束回合。 | 选择命令→查看合法范围与结果预览→确认执行；可打开背包、返回标题或主动放弃本轮。 | 每个完整命令结算后保存活动战斗。肉鸽模式不允许重开本战；非法指令保留入口并就近显示唯一原因。 |
| 15  <br/>战斗上下文叠层 | 右键动作菜单、悬停说明、移动路径、状态与轻量 Toast 的叠层样板。 | 右键打开当前格合法动作；悬停只读；双击或 Enter 确认，Esc 只关闭最上层可取消界面。 | 悬停、打开菜单和取消预览不保存也不扣费；提交动作后回到 14 号 HUD 并走统一命令链。 |
| 16  <br/>战后固定三选一 | F40／F41；显示来源、三项候选、完整效果、占位／栏位、构筑差异、选择态和背包不足原因。当前候选内容尚未冻结。 | 先选中再确认；可查看详情和整理行囊。前三场普通战的固定三选一必须选择 1 项，不能关闭或直接放弃。 | 空间不足保持待领取并进入 F41；去向、奖励、节点完成与解锁原子保存。其他奖励仅在配置明确允许时显示放弃。 |
| 17  <br/>单轮结算 | F60；显示已锁定的路线、完成节点、最终构筑、战斗／资源数据和人物记录。 | 查看完整档案；确认“结束本轮并返回入口”。 | 仍有战斗命令、待领奖励、背包处理或保存请求时不得进入。确认后保存总结、关闭活动单轮并返回 F00。 |
| 21  <br/>战斗结果变体 | F22／F61 的胜利与失败覆盖层；胜利显示任务结果、战损和奖励资格，失败显示原因、已发生战损和未获得内容。 | 胜利只可继续查看奖励；失败只可查看本轮结算，不能重试本战或领取未获得奖励。 | 胜利结果确认后进入 F40；死亡或主动放弃进入 F61。结果保存成功前不能离开或重复结算。 |
| 22  <br/>反馈、提示与焦点 | 全局／地图／战斗 Toast、术式／状态／敌人意图／地形悬停卡、阻塞原因和键盘焦点规范。 | Tab 移动焦点，Enter 确认，Esc 返回；悬停显示已冻结字段，禁用按钮保留并就近说明原因。 | 反馈层不改变玩法状态；同一时刻只显示最相关原因，不能用弹窗堆叠替代对象旁反馈。 |
| 23  <br/>战斗背包 | 已支付 1 行动点后的战斗整理界面；背包只存放装备与材料，显示搜索、筛选、占格、详情、槽位冲突和动作。 | 搜索、筛选、拖拽、旋转、装备／替换、调整战术栏、确认丢弃；锻造与术式专精不能在此完成。 | 打开时即支付 1 行动点，关闭不返还；合法变更随战斗状态保存。无合法位置时保留原对象并显示单一阻塞原因。 |

#### 常驻支持界面

| 稿号／页面 | 页面与显示职责 | 主要交互 | 出口、保存与阻塞 |
|-|-|-|-|
| 18  <br/>辅助设置 | 常驻声音、画面、动画强度、屏幕震动、战斗浮字、高对比、大号文字和键位提示。 | 逐项调整、恢复推荐值、保存并返回；战斗内打开时仍保持最高输入层。 | 选择“保存并返回”后写入本机设置；这些选项不改变玩法结算。 |
| 19  <br/>确认弹窗矩阵 | 覆盖存档、主动放弃本轮、保存并返回标题、丢弃高价值物品，以及配置允许时放弃奖励的不可逆确认。 | 默认焦点放在取消；危险按钮必须显示对象和不可逆后果；取消不改变任何状态。 | 肉鸽模式不出现“重开本战”或无损返回地图。固定三选一不出现放弃奖励；提交中禁止重复确认。 |
| 20  <br/>行程与行囊 | 当前档案总览；显示身份、出身、学院时序、路线、资源、装备、8 个术式位、4 个战术位和已读介绍。 | 查看分区详情、重看学院／单轮介绍、打开学院整备、返回原入口。 | 本页只读，不作为独立玩法存档；进入整备后的合法改动按 11A／11B 规则保存。 |

#### 本次补入的 Pixso 界面稿

下列 13 张为此前飞书正文未收录的页面；04、08、09、11A、11C、12、13、14、15、16、21 已保留在对应正文位置并按本次口径更新必要截图。

![02｜首启设置｜首次安装一次](https://feishu.cn/file/OpnHbrkh7oZ6Slxmv3Vc5BoKnXf)

![03｜世界观介绍｜第 1 页](https://feishu.cn/file/S2Zubtb5DozjfKxsnKVcg8aanxb)

![05｜固定出身三件套同页](https://feishu.cn/file/Q8EobEDZEouhiZx96f5c9sNqnib)

![06｜学院介绍｜每存档首次](https://feishu.cn/file/YMIqb8Av8oQsgtx6UVBcbTpSnkd)

![07｜单轮介绍｜每轮创建后](https://feishu.cn/file/Ig2Cbe6wqoed7FxMEAac9zKEnpb)

![10｜事件节点与选择结果](https://feishu.cn/file/O919bzhGeobpw4xdw1AceGLCnXb)

![11B｜学院整备｜术式与 4 格战术栏](https://feishu.cn/file/BixYb9kEJoY2Ujx1WsmcRQYPnae)

![17｜单轮结算](https://feishu.cn/file/QphQbpgrwoWv8TxG1bAcslotnpe)

![18｜辅助设置｜常驻入口](https://feishu.cn/file/ZVU0bwkMVoNLjKxo3WhcjpPJnsf)

![19｜确认弹窗状态矩阵](https://feishu.cn/file/DLvCbxz9QofY7YxqV7Tc0hoqnYb)

![20｜行程与行囊｜档案总览](https://feishu.cn/file/UY09boi5hosBZjxw1K0cGJ7xnGd)

![22｜Toast、提示与焦点状态](https://feishu.cn/file/Ep92bswldonRrcxoxnncwoC9n1b)

![23｜战斗背包｜整理与装备](https://feishu.cn/file/Vj14baJvqoOuoVxnnW2cOaXOnOb)
...
| FR-03 | 战斗1 | 完成入学资格验证或生命归零 | 胜利进入只读战果；失败进入 F61 | 按第 5.1.6 节 B1-01 至 B1-09 触发 |
