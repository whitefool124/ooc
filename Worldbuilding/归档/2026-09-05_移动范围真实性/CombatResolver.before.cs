    ...
    public static class CombatResolver
    {
        ...
        public static CombatEffectExecution Resolve(CombatState state, CombatCommand command)
        {
            ...
            switch (command.Type)
            {
                ...
                case CombatCommandType.OpenInventory: return ResolveOpenInventory(state, unit);
        ...
        private static CombatEffectExecution ResolveMove(CombatState state, UnitState unit, CombatCommand command)
        {
            ...
            IReadOnlyList<GridPosition> path = state.RainLanternCourt == null
                ? state.Map.FindShortestPath(unit.Position, command.Destination,
                    unit.MovementRangeThisTurn, position => state.IsOccupied(position, unit.Id))
                : state.RainLanternCourt.FindPath(state, unit, command.Destination);
