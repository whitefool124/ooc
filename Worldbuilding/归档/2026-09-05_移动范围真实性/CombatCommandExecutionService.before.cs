            ...
            try
            {
                ...
                IReadOnlyList<GridPosition> movementPath = command.Type == CombatCommandType.Move && commandUnit != null
                    ? state.RainLanternCourt == null
                        ? state.Map.FindShortestPath(commandUnit.Position, command.Destination, commandUnit.MovementRangeThisTurn,
                            position => state.IsOccupied(position, commandUnit.Id))
                        : state.RainLanternCourt.FindPath(state, commandUnit, command.Destination)
