...
> **当前实施进度（2026-09-09）：** 总案已采用新的第二组候选与统一伤害规则；当前运行时、数据表和自动测试仍是熔障校准旧效果、武器热载、兵装退火及单位／物件差分伤害，属于待同步实现。事件3、奖励发放、存档往返和工坊专精的既有流程可复用，但必须在新内容接入后重新完成 EditMode 回归与 Play Mode 操作、预览和视觉验收。
...
> **当前实施进度（2026-09-09）：** 本关地图、敌人编成、固定坐标、器材匣、学院储能芯、浅水与状态教学已有运行时基础；封门晶簇耐久、第二组技能、统一伤害预览和三条构筑反馈仍使用旧口径，必须随本轮总案同步实现并重新完成 EditMode 与 Play Mode 验收。
...
> **当前实施进度（2026-09-09）：** 第三组发放、精英地图、公开锁线、浅水、灯藤、干地卸压、精英奖励事务与商店解锁已有运行时基础；刻印战锤、稳压晶簇耐久、贯场冲压、回火导流和统一伤害仍使用旧口径，必须在同步后重新验证正常首次流程、存档、预览与表现。
