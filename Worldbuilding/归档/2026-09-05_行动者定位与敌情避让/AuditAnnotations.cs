...
public static class RenderReview
{
    ...
    public static string Run()
    {
        ...
        try
        {
            ...
            Func<string> signature=()=>string.Join("|",bootstrap.CurrentState.Units.Values.OrderBy(u=>u.Id).Select(u=>u.Id+":"+u.Position+":"+u.Health+":"+u.Shield+":"+u.Mana+":"+u.ActionPoints+":"+string.Join(",",u.Statuses.Select(s=>s.Key+"="+s.Value))));
            string beforePresentation=signature();
            ...
            return "stateUnchanged="+(beforePresentation==signature())+" lineImages="+temporary.GetRootGameObjects().SelectMany(root=>root.GetComponentsInChildren<Image>(true)).Count(i=>i.name.StartsWith("敌情引线_"))+"\n"+"crowded="+view.CrowdedIntentCount+"\n"+string.Join("\n",((IDictionary)Get(view,"cells")).Values.Cast<object>().Where(c=>(string)c.GetType().GetField("PresentedUnitId").GetValue(c)!=null).Select(c=>{
                var t=c.GetType();var r=(RectTransform)t.GetField("IntentRect").GetValue(c);var root=(GameObject)t.GetField("IntentRoot").GetValue(c);
                return t.GetField("PresentedUnitId").GetValue(c)+" intentVisible="+root.activeSelf+" intentPosition="+r.anchoredPosition;
            }))+"\nactiveMarker="+((RectTransform)Get(view,"activeFootprint")).anchoredPosition;
        }
        finally
        {
