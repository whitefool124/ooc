...
public static class RenderReview
{
    ...
    public static string Run()
    {
        ...
        try
        {
            ...
            var cellObjects=((IDictionary)Get(view,"cells")).Values.Cast<object>();
            var cell=cellObjects.First(c=>(string)c.GetType().GetField("PresentedUnitId").GetValue(c)=="enemy_1");
            var badge=(GameObject)cell.GetType().GetField("IntentRoot").GetValue(cell);
            var pointer=badge.GetComponents<MonoBehaviour>().First(c=>c.GetType().Name=="BattlefieldCellPointer");
            var primaryField=pointer.GetType().GetField("primaryClick",Flags);
            var originalClick=(Action<GridPosition,int>)primaryField.GetValue(pointer);
            GridPosition clicked=new GridPosition(-1,-1);
            primaryField.SetValue(pointer,new Action<GridPosition,int>((p,n)=>{clicked=p;originalClick(p,n);}));
            var data=new UnityEngine.EventSystems.PointerEventData(UnityEngine.EventSystems.EventSystem.current);
            data.button=UnityEngine.EventSystems.PointerEventData.InputButton.Left;data.clickCount=1;
            var badgeRect=badge.GetComponent<RectTransform>();
            Vector3 world=badgeRect.TransformPoint(badgeRect.rect.center);
            data.position=camera.WorldToScreenPoint(world);
            var hits=new System.Collections.Generic.List<UnityEngine.EventSystems.RaycastResult>();
            badge.GetComponentInParent<GraphicRaycaster>().Raycast(data,hits);
            bool hit=hits.Any(h=>h.gameObject==badge);
            UnityEngine.EventSystems.ExecuteEvents.Execute(badge,data,UnityEngine.EventSystems.ExecuteEvents.pointerClickHandler);
            var board=(RectTransform)Get(view,"boardRect");Vector3 boardPoint=board.InverseTransformPoint(world);
            var underlying=new GridPosition(Mathf.FloorToInt(boardPoint.x/64),8-Mathf.FloorToInt(-boardPoint.y/64));
            return "badgeRaycast="+hit+" clickedSource="+clicked+" actualEnemy="+bootstrap.CurrentState.GetUnit("enemy_1").Position+" underlyingCell="+underlying+" heroPosition="+bootstrap.CurrentState.GetUnit("hero").Position;
        }
        finally
        {
