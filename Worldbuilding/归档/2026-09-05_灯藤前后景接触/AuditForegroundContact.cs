...
public static class RenderReview
{
    ...
    public static string Run()
    {
        ...
        try
        {
            ...
            var feedbackObject=new GameObject("VineContactProbe");
            var feedback=feedbackObject.AddComponent<CombatVisualFeedback>();feedback.enabled=false;
            var playback=(CombatActionPlayback)Get(feedback,"actionPlayback");
            var presenter=new CombatBattlefieldCellPresenter(new BattlefieldPresentationAdapter(),(CombatFormalVisualAssets)Get(bootstrap,"formalAssets"));
            Func<BattlefieldCellPresentation> presented=()=>presenter.Build(bootstrap.CurrentState,
                FirstRegionLevelCatalog.For(bootstrap.CurrentLevelId),null,new CombatSelectionController(),false,feedback,
                heroPosition,_=>null,(_,__)=>null,_=>null,_=>null);
            var capture=playback.Capture(bootstrap.CurrentState,null,"hero",0);
            capture.SetImpact(heroPosition,.3f);
            tile.IsLampVine=false;tile.IsScorched=true;
            playback.Start(capture,bootstrap.CurrentState,Time.unscaledTime+10,_=>{});
            var beforeHit=presented();
            Call(view,"RefreshCell",heroCell,beforeHit,bootstrap.BattlefieldViewport,false);
            bool beforeHitVisible=beforeHit.ObjectForegroundRows==10&&heroFront.gameObject.activeSelf;
            playback.Clear();playback.Start(capture,bootstrap.CurrentState,Time.unscaledTime-10,_=>{});
            var afterHit=presented();
            Call(view,"RefreshCell",heroCell,afterHit,bootstrap.BattlefieldViewport,false);
            bool afterHitHidden=afterHit.ObjectForegroundRows==0&&!heroFront.gameObject.activeSelf;
            playback.Clear();tile.IsLampVine=originalVine;tile.IsScorched=originalScorch;
            Call(view,"RefreshCell",heroCell,model,bootstrap.BattlefieldViewport,false);Call(view,"RefreshUnitOrder");
            ...
            return "frontImages="+frontCells.Length+" passThrough="+passThrough+" fixedTerrain="+stationary+
                " groundDepth="+groundDepth+" movingDepth="+depthCorrect+" burnedRemoved="+removed+" restoredReused="+reused+" beforeHitVisible="+beforeHitVisible+" afterHitHidden="+afterHitHidden+" emptyCompositionPixelDifferences="+differences;
        }
        finally
        {
