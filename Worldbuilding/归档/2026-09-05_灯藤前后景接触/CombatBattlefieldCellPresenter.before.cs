    ...
    public sealed class CombatBattlefieldCellPresenter
    {
            ...
            Func<UnitState, EnemyIntentPresentation> enemyIntent)
        {
            ...
            CombatMovementPose travel = feedback != null ? feedback.UnitTravelPose(unit) :
                new CombatMovementPose(new Vector2(position.X, position.Y), unit?.Facing ?? Facing.South);
            ...
            string hover = unit == null ? BuildTerrainHover(state, fireBattle, tile, position) : unit.IsHero
                ...
                : CombatInformationPresenter.BuildEnemyHoverDetails(state, unit, intent) +
                  FirstBattleEnemyRuleText(state, unit) +
            ...
            return new BattlefieldCellPresentation(position, floor, floorUv, floorRotation,
                ...
                objectLabelColor, loot, unit, vitals, statuses, intent, intentTexture, hover, travelOffset, travel.Facing);
        }
            ...
            GridPosition position)
        {
            ...
            if (tile.Cover == CoverType.Heavy)
                return tile.IsDestroyed ? "重掩体残骸\n已失去阻挡和防护效果。" : "重掩体\n耐久 " + tile.Durability + "；阻挡移动与视线，肉鸽战斗中正前方相邻时于自身回合结束获得 4 护盾。";
            if (tile.IsDevice)
        ...
        private static string FirstBattleEnemyRuleText(CombatState state, UnitState unit)
        {
            if (state?.RainLanternCourt == null || unit == null) return string.Empty;
            if (unit.EnemyArchetypeId == "tether_hound")
                return "\n搜索规则：看见主角时追击并优先扑咬；失去视线后会前往灯藤入口，完整嗅探一回合，再沿外围顺时针搜索。";
            if (unit.EnemyArchetypeId == "pyromancer")
