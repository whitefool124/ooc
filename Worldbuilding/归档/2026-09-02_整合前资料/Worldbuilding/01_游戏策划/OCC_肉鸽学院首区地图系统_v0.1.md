...
> 状态：学院地图现行基线；核心拓扑已实装，最新时序、资源、事件池与固定首领规则仍有迁移债。
...
- 没有隐藏倒计时、追击、敌情推进、无预警关门或强制扣血。风险来自战斗后继承的生命、个人魔力、战术道具剩余次数、具体钥匙实例、核心许可、构筑状态与公开的学院时序；普通护盾不跨战，运营以太不是货币或地图资源。
- 节点、敌群、事件结果和奖励候选由种子确定；一旦展示或完成，不会在回访时重掷。
- 区域门可使用钥匙、权限卡或学院许可开启；它们是空间/资源门，不是时间门。
- 节点数量由玩家自由选择，不设置“实训名额”或地图选点上限。地图移动、回访、商店、工坊、休整、宝藏、查看信息与整理背包均不推进**学院时序**；只有战斗和事件按确认前显示的固定成本推进时序。
- 取得 2 枚核心许可且至少处理 12 个节点后，玩家可以提前进入首领。约 20 个节点是首区正常构筑节奏，不是门槛或上限。
- 学院时序是公开的世界时间压力：0–20 为正常入学期；达到 21 点进入“学期收束”；25–27 点显示明确的首领阶段提示；达到或跨过 28 点时，已确认节点按完整成本结算并允许一次自然超出，随后关闭其他耗时节点、直接开放首领。强制首领阶段不再用 `12 节点 + 2 许可` 阻塞；战斗／事件结算前必须预览跨阈值结果。

### 2.1 首区门槛首轮调优决定（2026-08-14）

- 首领入口在时序低于 28 时启用既定的“至少 12 个已探索节点 + 2 枚核心许可”提前挑战门槛；地图顶栏和首领详情必须分别显示当前值、目标值与缺口，不能只显示通用锁定。进入强制首领阶段后该门槛解除。
- 固定拓扑求解已确认至少存在三类不同许可组合的首领路线；首轮实验不调整 `12/2` 数值，也不改地图连线、奖励或资源价格。
- 时序达到 21 进入收束、达到或跨过 28 进入强制首领阶段。首领胜利后先提供一次贡献消费与背包整理窗口，再由玩家确认转入；下一人生阶段运行时尚未接入期间，胜利结算只能显示“转入待实现”，不得让玩家返回其他耗时节点。
...
| 事件槽 | 8 | 从 16–20 条事件池按种子无重复抽取 | 公开取舍；可给资源／许可，或进入已预览额外战斗 |
| 服务节点 | 4 | 校内补给处、校准工坊、医务室/休整室 | 购买、替换/校准、治疗；只服务战斗循环 |
| 宝藏 | 2 | 封存器材库/遗失物库 | 稀有卷轴、法宝或钥匙 |
| 首领 | 1 | 学院封存塔核心 | 本区最终战斗与胜负结算 |

总数为 40。玩家在进入强制首领阶段前可自由选择节点数量；通常约 20 个节点形成足够构筑并挑战首领，过度探索会在时序达到或跨过 28 后锁定其他耗时节点并进入首领阶段。节点数量可在后续内容生产中替换同类节点，但不得以额外非战斗生活活动挤占普通、精英或首领战斗预算。
...
- **器材登记：** 支付公开的金币或学院贡献，购买确定的满次数战术道具新实例、卷轴、情报服务或许可办理；已有实例不能按次补充，余额不足不能选择。
- **异常传闻：** 支付侦测信标显示附近节点精确内容，或进入指定战斗获取宝藏线索。

事件确认框必须显示成本、奖励、是否开战、战斗目标与失败后果。主角存活但任务失败时，节点关闭、状态和已消耗道具次数保留、装备不受损并降低收益；不得重试刷恢复。禁止加入关系数值、日程、学段、毕业去向、随机扣血或无预览惩罚。
...
- 地图 UI 显示：40 节点拓扑、当前学院时序与下一阶段转入阈值、当前生命／个人魔力、金币、学院贡献、当前养成装备、战术道具剩余次数、已清安全状态、区域门、节点类型、风险等级及事件预览入口。普通护盾不跨战，因此不作为地图资源显示。养成装备不显示耐久。战斗确认页和事件选项必须显示时间成本、生命／个人魔力预计恢复及跨阈值结果；零时间节点明确显示“不消耗时间”。保持 1920×1080 左图 75% / 右 HUD 25% 基准。
- 肉鸽独立存档必须恢复节点拓扑、访问／完成、区域门、金币、学院贡献、生命、个人魔力、6×10 背包、构筑、已展示事件结果和固定首领进度；不得读写剧情存档或保存跨节点普通护盾。
...
4. 无追击、无预警关门、日程／学段、关系养成、毕业路线或非战斗人生属性；学院时序仅由战斗和事件的公开时间成本推进，地图走动与非战斗功能节点均为零时间，阶段转入阈值和剩余时序必须始终公开。
