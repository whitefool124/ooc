...
## M-A46 战术范围覆盖重制（2026-09-02）

| 资产组 | 数量 | 尺寸／导入 | 状态 |
| --- | ---: | --- | --- |
| 移动／攻击范围角标 | 2 | `32×32`、硬 Alpha、Point、Clamp、PPU32 | FORMAL / QA_PASS |

- 移动范围使用冷青测绘角标，攻击范围使用锈红威胁角标；二者保持开放中心，并以 `2px` 透明安全边避免相邻地块在公共顶点堆叠成星形。
- 当前模式范围保持约 `.88` 主亮度，非当前范围降为 `.22–.26`；运行时补入淡入淡出与轻微透明度脉冲，不做平滑缩放或模糊发光。
- 两份独立原料、manifest、1×／4×／灰阶／棋盘格与接触证据位于 `正式美术生产/M-A46/`；Unity 1920×1080、960×540、攻击模式与右键菜单接触均已归档。机器验证 `2/2 PASS`、合同审计 `PASS`、全量 EditMode `694/694`、PlayMode `1/1`。
