...
namespace OCC.Combat.Presentation
{
    public sealed class FormalRogueliteUi : MonoBehaviour
    {
        ...
        private void DrawLanding()
        {
            ...
            Label("标题", "固定首次体验", card.transform, new Vector2(64, -44), new Vector2(1180, 96), 42, text, TextAnchor.MiddleLeft);
            Text description = Label("说明", "护障检修户 · 就地接线 · 借障导流\n每个存档首次经历 10 个内容节点与 4 场战斗；完成后再进入常驻学院内容池。", card.transform, new Vector2(66, -130), new Vector2(1180, 76), 22, muted, TextAnchor.UpperLeft);
            FormalUiKit.PreventAutomaticWrapping(description);
            ActionButton("开始首次体验", "固定出身与教学路径；不选择开局流派", card.transform, new Vector2(64, -220), new Vector2(1192, 112), cyan, true, () => bootstrap.RequestStartMapRoguelite(false, FireRogueliteStarterCatalog.Universal));
        ...
        private void DrawNodeRoomActions(Transform parent, RogueliteMapRun run, RogueliteMapNode node, bool current, bool cleared, Color accent)
        {
            ...
            {
                Label("页面标题", "确认固定出身", parent, new Vector2(44, -38), new Vector2(1000, 48), 32, text, TextAnchor.MiddleLeft);
                Label("页面说明", "护障检修户 · 就地接线 · 借障导流", parent, new Vector2(44, -104), new Vector2(1000, 64), 22, cyan, TextAnchor.UpperLeft);
                ActionButton("确认并解锁第一战", "此选择属于固定教程，不消耗资源。", parent, new Vector2(224, -224), new Vector2(720, 126), cyan, true,
                    bootstrap.AcknowledgeFirstRunOrigin, iconPath: FormalArtRegistry.NavigationPath("confirm"));
        ...
        private void DrawCombatActionDossier(Transform parent, RogueliteMapRun run, RogueliteMapNode node, bool current, Color accent)
        {
            ...
            RogueliteEncounterDefinition encounter = run.IsFirstRunExperience && node.Id != "B1" ? null : RogueliteEncounterCatalog.For(run, node.Id);
            ...
            bool contentReady = !run.IsFirstRunExperience || node.Id == "B1";
            string enterReason = !contentReady ? "阶段 A 已留好接口；地图、敌人和术式内容尚未定稿。" : canEnter ? "准备好就出发" : RogueliteMapVisualPresentation.RestrictionText(run, node);
            GameObject start = ActionButton(contentReady ? "出发" : "等待内容接入", enterReason, parent, new Vector2(44, -516), new Vector2(672, 126), canEnter && contentReady ? accent : muted, canEnter && contentReady,
        ...
        private void DrawSpellLoadout(Transform parent, RogueRunDto dto, IReadOnlyDictionary<string, SpellDefinition> spells)
        {
            ...
            for (int index = 0; index < RogueRuntimeConstants.SpellSlotCount; index++)
            {
                ...
                GameObject slot = ActionButton((index + 1) + "  " + name, detail, spellPanel.transform,
                    new Vector2(28 + (index % 2) * 506, -112 - (index / 2) * 116), new Vector2(478, 104), selected ? amber : cyan, true,
                    () => { selectedLoadoutSpellIndex = slotIndex; Invalidate(false); }, iconPath: RogueSpellIconPath(id));
                if (spell != null) BindHover(slot, name, SpellTooltipBody(spell), selected ? amber : cyan);
            }
        ...
        private void DrawSpellDetails(Transform parent, RogueRunDto dto, IReadOnlyDictionary<string, SpellDefinition> spells, Vector2 position)
        {
            ...
            Label("术式提示", "战斗中按对应数字键选择；不可在战斗中改装。", detailPanel.transform, new Vector2(32, -548), new Vector2(540, 48), 16, muted, TextAnchor.MiddleLeft);
        }
