...
public static class RenderReview
{
    ...
    const bool LEGACY_LAYOUT = false;
    ...
    static Color FeedbackColor(CombatFeedbackKind kind) { ColorUtility.TryParseHtmlString(CombatFeedbackCatalog.For(kind).ColorHex, out Color color); return color; }
    static void CallStatic(object target, string method, params object[] args) { try { Call(target, method, args); } catch (TargetInvocationException ex) { if (!(ex.InnerException is NullReferenceException)) throw new Exception(ex.InnerException.ToString()); if (!ex.InnerException.StackTrace.Contains("DG.Tweening") && !ex.InnerException.StackTrace.Contains("ShowFloatingText") && !ex.InnerException.StackTrace.Contains("ShowDamagePopup")) throw; } }
    public static string Run()
    {
        ...
        bool oldContrast = FormalUiTheme.HighContrastEnabled, oldLarge = FormalUiTheme.LargeTextEnabled;
        ...
        try
        {
            ...
            FormalUiTheme.ConfigureAccessibility(bootstrap.UiPreferences.HighContrast, bootstrap.UiPreferences.LargeText);
            ...
            var feedback = go.AddComponent<CombatVisualFeedback>(); Set(feedback, "bootstrap", bootstrap); feedback.enabled = false;
            var fc = FormalUiKit.CanvasRoot("ReviewFeedback", 60); Set(feedback, "canvas", fc);
            var clip = new GameObject("ReviewClip", typeof(RectTransform), typeof(RectMask2D)).GetComponent<RectTransform>();
            clip.SetParent(fc.transform, false); clip.anchorMin = clip.anchorMax = new Vector2(.5f,.5f);
            var vp = bootstrap.CurrentBattlefieldViewport;
            clip.anchoredPosition = new Vector2(vp.X + vp.Width*.5f-960f,540f-vp.Y-vp.Height*.5f);
            clip.sizeDelta = new Vector2(vp.Width,vp.Height); Set(feedback, "battlefieldClip", clip);
            var target = bootstrap.CurrentState.GetUnit("hero").Position;
            CallStatic(feedback, "ShowFloatingText", target, "束缚 2", FeedbackColor(CombatFeedbackKind.Bound), "bound");
            CallStatic(feedback, "ShowFloatingText", target, "破甲 2", FeedbackColor(CombatFeedbackKind.ArmorBreak), "armor_break");
            CallStatic(feedback, "ShowFloatingText", target, "迟缓 2", FeedbackColor(CombatFeedbackKind.Slow), "slow");
            CallStatic(feedback, "ShowDamagePopup", new CombatFeedbackEvent(CombatFeedbackKind.Damage, target, 6));

            if (LEGACY_LAYOUT) foreach (Transform item in clip) {
                var rect = item as RectTransform;
                if (item.name == "伤害反馈") rect.anchoredPosition = (Vector2)Call(feedback,"FloatingFeedbackPosition",target);
                if (item.name == "实际伤害跳字") rect.anchoredPosition = (Vector2)Call(feedback,"DamagePopupPosition",target,0);
                foreach (var child in item.GetComponentsInChildren<Transform>()) if(child.name=="反馈边线") child.gameObject.SetActive(false);
            }
            if(clip.GetComponentsInChildren<Text>().Length != 4) throw new Exception("Expected all four feedback labels");
            ...
            var protectedRects = new System.Collections.Generic.List<Rect>(); view.AppendFeedbackObstacles(protectedRects);
            if (protectedRects.Count == 0) throw new Exception("Expected visible unit obstacles");
            float overlap = 0; var placements = (IList)Get(feedback, "feedbackPlacements");
            foreach(var placement in placements) {
                var labelRect=(RectTransform)placement.GetType().GetField("Rect").GetValue(placement);
                var targetGrid=(GridPosition)placement.GetType().GetField("Target").GetValue(placement);
                var damageOwner=bootstrap.CurrentState.Units.Values.FirstOrDefault(u=>u.Position==targetGrid);
                protectedRects.Clear(); view.AppendFeedbackObstacles(protectedRects,labelRect.sizeDelta.x==240f,damageOwner?.Id);
                var swept = (Rect)placement.GetType().GetField("Swept").GetValue(placement);
                foreach(var obstacle in protectedRects) overlap += Mathf.Max(0,Mathf.Min(swept.xMax,obstacle.xMax)-Mathf.Max(swept.xMin,obstacle.xMin))*Mathf.Max(0,Mathf.Min(swept.yMax,obstacle.yMax)-Mathf.Max(swept.yMin,obstacle.yMin));
            }
            return "obstacles="+protectedRects.Count+"|protectedOverlap="+overlap+"|popups="+placements.Count+"|"+width + "|" + height + "|" + CombatUnitHudLayout.UnitPresentationRect(bootstrap.BattlefieldViewport.CellRect(hound.Position)).width + "|" + shown.name + "|" + shown.width + "x" + shown.height + "|" + shown.filterMode + "|" + shown.wrapMode + "\n" + Convert.ToBase64String(pixels.EncodeToPNG());
        }
        finally
        {
            FormalUiTheme.ConfigureAccessibility(oldContrast, oldLarge);
