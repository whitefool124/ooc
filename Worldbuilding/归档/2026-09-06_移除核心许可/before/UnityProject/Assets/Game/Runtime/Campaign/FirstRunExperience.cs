...
namespace OCC.Combat
{
    ...
    public sealed class FirstRunExperienceState
    {
        ...
        public void ClaimEliteReward()
        {
            RequireMutable();
            if (Outcome != FirstRunOutcome.EliteVictory || EliteRewardClaimed) throw new InvalidOperationException("Elite reward is not available.");
        ...
        public void ChooseEvent(string nodeId, string optionId)
        {
            ...
            {
                ForgeMaterialCount++;
                GrantedRelicSlotIds.Add("PHASE_A_PLACEHOLDER_RELIC_SLOT_1");
            }
            else if (nodeId == "EV2") AcademyFoodCount++;
            else if (nodeId == "EV3")
            {
                SpecializationMaterialCount++;
                if (optionId.EndsWith("RELIC", StringComparison.Ordinal)) GrantedRelicSlotIds.Add("PHASE_A_PLACEHOLDER_RELIC_SLOT_2");
            }
            SetCompleted(nodeId);
            RecomputeNodeFlags();
        }
        ...
        private bool IsUnlocked(string id)
        {
            ...
            switch (id)
            {
                ...
                case "S": return Completed("X") && Outcome == FirstRunOutcome.EliteVictory && EliteRewardClaimed;
        ...
        private bool RewardClaimedFor(string nodeId)
        {
            ...
            return Completed(nodeId) && reward != null && !string.IsNullOrEmpty(reward.SelectedId);
        }
    ...
    {
        public const string ContentVersion = "first-run-v1-b1-formal";
        ...
        {
            new RogueliteMapNode("O", RogueliteMapNodeType.Start, "护障检修户", "确认固定出身三件套。", 0, 2, 0, 0, "B1"),
            new RogueliteMapNode("B1", RogueliteMapNodeType.Combat, "雨后灯庭", "入学资格验证：击倒缚环寻迹兽与高年级火矢生。", 1, 2, 0, 0, "O", "B2", "EV1", "EV2"),
            new RogueliteMapNode("EV1", RogueliteMapNodeType.Event, "事件 1", "锻造材料与第一件法宝槽。", 1, 1, 0, 0, "B1"),
            new RogueliteMapNode("EV2", RogueliteMapNodeType.Event, "事件 2", "学院食材与资源选择。", 1, 3, 0, 0, "B1"),
            new RogueliteMapNode("B2", RogueliteMapNodeType.Combat, "普通战斗 2", "阶段 A 固定遭遇槽。", 2, 2, 0, 0, "B1", "B3", "W", "EV3"),
            new RogueliteMapNode("W", RogueliteMapNodeType.Workshop, "工坊", "一次锻造与一次术式专精。", 2, 1, 0, 0, "B2"),
            new RogueliteMapNode("EV3", RogueliteMapNodeType.Event, "事件 3", "专精材料与资源／法宝选择。", 2, 3, 0, 0, "B2"),
            new RogueliteMapNode("B3", RogueliteMapNodeType.Combat, "普通战斗 3", "阶段 A 固定遭遇槽。", 3, 2, 0, 0, "B2", "X", "M"),
            new RogueliteMapNode("M", RogueliteMapNodeType.Medical, "医务室", "健康确认、治疗与固定餐食。", 3, 3, 0, 0, "B3"),
            new RogueliteMapNode("X", RogueliteMapNodeType.Elite, "第一场精英战", "可能致死的固定精英遭遇槽。", 3, 1, 0, 0, "B3", "S"),
            new RogueliteMapNode("S", RogueliteMapNodeType.Shop, "精英后商店", "首次展开即完成固定教学段。", 4, 1, 0, 0, "X")
        };
        ...
        public static FirstRunExperienceState CreatePhaseA()
        {
            ...
            state.RewardGroups.Add(Reward("REWARD-GROUP-1", "B1"));
            state.RewardGroups.Add(Reward("REWARD-GROUP-2", "B2"));
            state.RewardGroups.Add(Reward("REWARD-GROUP-3", "B3"));
            state.Events.Add(new FirstRunEventSnapshot("EV1", new[] { "PHASE_A_PLACEHOLDER_EVENT_EV1_CONFIRM" }));
            state.Events.Add(new FirstRunEventSnapshot("EV2", new[] { "PHASE_A_PLACEHOLDER_EVENT_EV2_CONTRIBUTION", "PHASE_A_PLACEHOLDER_EVENT_EV2_GOLD" }));
            state.Events.Add(new FirstRunEventSnapshot("EV3", new[] { "PHASE_A_PLACEHOLDER_EVENT_EV3_CONTRIBUTION", "PHASE_A_PLACEHOLDER_EVENT_EV3_RELIC" }));
            state.Encounters.Add(new FirstRunEncounterSnapshot("B1", RainLanternCourtRuntime.LevelId,
                ...
                Array.Empty<string>()));
            state.Encounters.Add(Encounter("B2", new[] { "MECHANIC-SLOT-B", "MECHANIC-SLOT-C" }, true));
            state.Encounters.Add(Encounter("B3", new[] { "MECHANIC-SLOT-A", "MECHANIC-SLOT-C" }, true));
            state.Encounters.Add(Encounter("X", Array.Empty<string>(), false));
        ...
        private static FirstRunRewardGroupSnapshot Reward(string id, string nodeId) =>
            new FirstRunRewardGroupSnapshot(id, nodeId, Enumerable.Range(1, 3).Select(index => "PHASE_A_PLACEHOLDER_" + id + "_OPTION_" + index));

        private static FirstRunEncounterSnapshot Encounter(string nodeId, IEnumerable<string> mechanics, bool buildFeedback)
        {
            string prefix = "PHASE_A_PLACEHOLDER_" + nodeId;
            return new FirstRunEncounterSnapshot(nodeId, prefix + "_LEVEL", prefix + "_ENEMY_SCRIPT", prefix + "_DROP",
                prefix + "_DIALOGUE", mechanics, Enumerable.Range(1, 3).Select(index => prefix + "_ROUTE_" + index),
                buildFeedback ? new[] { "PHASE_A_PLACEHOLDER_NEW_BUILD_FEEDBACK" } : Array.Empty<string>());
        }
    }
    ...
    public static class FirstRunExperienceCodec
    {
        public static string Serialize(FirstRunExperienceState state)
        {
            ...
            List<string> lines = new List<string>
            {
                Row("H", state.ContentVersion, state.Lifecycle.ToString(), state.Outcome.ToString(), state.CurrentNodeId,
                    ...
                    state.SpecializationMaterialCount.ToString(), state.AcademyFoodCount.ToString()),
            ...
            lines.AddRange(state.RewardGroups.OrderBy(value => value.Id, StringComparer.Ordinal).Select(value => Row("R", value.Id, value.NodeId, value.SelectedId, List(value.CandidateIds))));
        ...
        public static FirstRunExperienceState Deserialize(string raw)
        {
            ...
            state.Medical.MealCandidateIds.Clear(); state.Shop.Offers.Clear(); state.GrantedRelicSlotIds.Clear();
            foreach (string line in raw.Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries))
            {
                string[] f = line.Split('\t');
                string kind = f[0];
                if (kind == "H" && f.Length == 11)
                {
                ...
                else if (kind == "R" && f.Length == 5) { FirstRunRewardGroupSnapshot value = new FirstRunRewardGroupSnapshot(U(f[1]), U(f[2]), ParseList(f[4])); value.SelectedId = U(f[3]); state.RewardGroups.Add(value); }
