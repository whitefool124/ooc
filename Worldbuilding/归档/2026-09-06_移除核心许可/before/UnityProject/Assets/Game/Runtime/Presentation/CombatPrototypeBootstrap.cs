    ...
    public sealed class CombatPrototypeBootstrap : MonoBehaviour, ICombatPresentationCompositionHost, ITacticalHudHost, ICombatActionPresentationHost
    {
        ...
        private void RunEnemyTurn()
        {
            ...
            try
            {
                ...
                {
                    bool continueFormalFirstBattleTurn = state.RainLanternCourt != null &&
                        state.ActiveUnitId == enemy.Id && enemy.ActionPoints > 0 &&
                        advance.CommandType != CombatCommandType.EndTurn;
                    if (state.ActiveUnitId == enemy.Id && !continueFormalFirstBattleTurn)
        ...
        private void FollowHeroAtSafeEdge()
        {
            ...
            CombatMovementPose pose = visualFeedback?.UnitTravelPose(hero) ??
                new CombatMovementPose(new Vector2(hero.Position.X, hero.Position.Y), hero.Facing);
