    ...
    public static class RogueliteMapRunValidator
    {
        ...
        private static void ValidateFirstRun(RogueliteMapRun run, RogueliteMapRunValidationResult result)
        {
            ...
            if (state.RewardGroups.Count != 3 || state.RewardGroups.Any(group => group.CandidateIds.Count != 3 ||
                ...
                (!string.IsNullOrEmpty(group.SelectedId) && !group.CandidateIds.Contains(group.SelectedId)))) result.Add("first_run.reward_contract");
            ...
            for (int index = 0; index < battles.Length; index++)
            {
                ...
                if ((!formalB1 && !pairs[index].SequenceEqual(encounter.MechanicSlotIds)) || encounter.IdealRouteIds.Count < 3)
            ...
            if (state.Shop.Opened && (!state.EliteRewardClaimed || state.Outcome != FirstRunOutcome.EliteVictory)) result.Add("first_run.shop_gate");
