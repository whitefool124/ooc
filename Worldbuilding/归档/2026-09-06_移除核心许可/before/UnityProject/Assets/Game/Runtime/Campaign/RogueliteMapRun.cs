...
namespace OCC.Combat
{
    ...
    public static class RogueliteMapCatalog
    {
        ...
        public static readonly IReadOnlyList<RogueliteMapNode> Nodes = new[]
        {
            ...
            new RogueliteMapNode("med_bay", RogueliteMapNodeType.Medical, "行军医帐", "治疗与餐食服务。", 3, 3, 0, 0, "relay_raid", "field_workshop", "relay_event", "permit_archive", "sealed_market"),
            ...
            new RogueliteMapNode("core_vault", RogueliteMapNodeType.Event, "学院封存库", "管理员允许你从封存柜里带走一件东西。", 6, 2, 1, 0, "aether_refinery", "safety_room", "core_approach", "core_finale"),
            ...
            new RogueliteMapNode("tower_lift", RogueliteMapNodeType.Event, "封存管理员匣", "管理员让你在稀有法宝和核心许可之间选一个。", 7, 4, 0, 0, "tower_records", "tower_foyer")
        };
    ...
    public sealed class RogueliteMapRun
    {
        ...
        public bool IsComplete => IsFirstRunExperience
            ? FirstRunExperience.Lifecycle == FirstRunLifecycle.ShopOpened || FirstRunExperience.IsTerminal
        ...
        public int CorePermits => IsFirstRunExperience ? (FirstRunExperience.EliteRewardClaimed ? 1 : 0) : completed.Count(id => RogueliteMapCatalog.Node(id).GrantedAccessCards > 0) +
        ...
        public bool CanChallengeAcademyFinale => StageTime >= AcademyMapTuning.TransitionProgress ||
            (CompletedAcademyNodeCount >= AcademyMapTuning.BossMinimumProgress && CorePermits >= AcademyMapTuning.CorePermitRequirement);
        ...
        public IReadOnlyList<string> CurrentFirstRunRewardIds
        {
            get
            {
                ...
                if (FirstRunExperience.Outcome == FirstRunOutcome.EliteVictory && !FirstRunExperience.EliteRewardClaimed)
                    return new[] { "PHASE_A_PLACEHOLDER_X_REWARD" };
        ...
        public IReadOnlyList<RogueliteReward> CurrentRewards
        {
            get
            {
                ...
                if (IsFirstRunExperience) return Array.Empty<RogueliteReward>();
        ...
        public IReadOnlyList<RogueliteNodeContentChoice> CurrentContentChoices
        {
            get
            {
                ...
                if (IsFirstRunExperience)
                {
                    ...
                    return currentEvent.OptionIds.Select(id => new RogueliteNodeContentChoice(id, "阶段 A 公开选项", "内容待冻结；本次只验证选择、结算与存档。", RogueliteNodeContentEffect.Intelligence)).ToArray();
                }
        ...
        public void ClaimReward(string rewardId)
        {
            if (IsFirstRunExperience)
            {
                if (FirstRunExperience.Outcome == FirstRunOutcome.EliteVictory && !FirstRunExperience.EliteRewardClaimed)
                {
                    if (!string.Equals(rewardId, "PHASE_A_PLACEHOLDER_X_REWARD", StringComparison.Ordinal))
                        throw new InvalidOperationException("Elite reward is not available.");
                    FirstRunExperience.ClaimEliteReward();
                }
                else FirstRunExperience.ClaimReward(rewardId);
        ...
        private void SyncFirstRunProjection()
        {
            ...
            AwaitingReward = !string.IsNullOrEmpty(FirstRunExperience.PendingRewardGroupId) ||
                FirstRunExperience.Outcome == FirstRunOutcome.EliteVictory && !FirstRunExperience.EliteRewardClaimed;
            if (rogueRunDto != null)
            {
