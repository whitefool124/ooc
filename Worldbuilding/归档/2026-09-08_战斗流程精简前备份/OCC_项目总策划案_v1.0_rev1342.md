...
1. 进入战斗前，系统先检查地图、出生点、敌人和任务目标。配置有问题时不进入战斗，并直接说明原因。
2. 战斗加载后，先显示任务目标、全部敌人、场地和敌人第一条公开意图，然后才开放操作。
3. 行动条自动推进。谁先达到 100，谁先获得回合；单位行动时，其他人的行动条暂停。
4. 轮到主角时，行动点重置为 3。玩家可以连续移动、攻击、使用术式、使用战术道具、互动或整理背包。
5. 每次行动都先选动作和目标，再看范围、花费和结果。确认后扣除资源并完成结算。
6. 一次行动结束后立即检查死亡和任务目标。战斗没有结束且还有行动点时，玩家可以继续行动，也可以主动结束回合。
7. 主角结束回合后清空剩余行动点。敌人按公开意图各执行一个动作，然后继续推进下一名单位。
8. 达成任务目标或主角死亡、主动放弃时，马上锁住战斗操作。胜利进入战果和奖励，失败进入本轮结算。
...
1. 回合开始先看任务目标、敌人意图、自己的状态和剩余行动点。
2. 从底栏选择移动、武器、术式、互动或战术道具；需要换装时可以打开战斗背包。
3. 在战场上选择目标或格子。界面会显示能不能用、要花多少资源，以及执行后的主要结果。
4. 不能执行时不扣资源，按钮或目标旁会直接说明原因。玩家可以换目标、换动作或取消。
5. 确认后暂时锁住其他操作，依次播放移动、攻击、状态和场地反馈，完成后刷新数值、行动条和战斗日志。
6. 还有行动点并且战斗未结束时，可以继续选择动作。选择结束回合后，剩余行动点清空并交给下一名单位。
...
![战斗界面](https://feishu.cn/file/BRBTb1R7hoOnUKxun24c9Ya3nOj)

**界面说明：**左边是战场，右边是单位和行动信息，底部是移动、武器、术式、互动、战术栏和结束回合。

**交互规则：**先选动作，再看范围和结果，最后确认执行。不能使用的动作保留按钮，并直接说明原因。完整操作见 3.1.6.3，快捷键见 5.2。
...
**交互规则：**右键打开当前对象能做的事；悬停只看说明。Esc 只关闭最上层，取消预览不扣资源。
...
**交互规则：**打开时消耗一点行动点，关闭不返还。可以整理和换装，但不能在这里锻造或专精术式。
...
| 输入 | 操作 |
|-|-|
| 左键 | 选择单位、格子或指令，并查看行动预览 |
| 双击可达格 | 确认快捷移动 |
| 右键 | 打开目标位置当前能做的事情 |
| 鼠标悬停 | 查看单位、术式、状态、敌人意图和地形说明 |
| 滚轮 | 缩放战场 |
| 中键拖动 | 拖动战场画面 |
| 空格＋左键拖动 | 拖动战场画面 |
...
| 按键 | 什么时候用 | 操作 |
|-|-|-|
| Tab | 任何战斗界面 | 把焦点移到下一个按钮或选项 |
| 数字键 1–8 | 战斗主界面 | 选择对应术式 |
| B | 战斗主界面 | 打开或关闭战斗背包；打开时按规则消耗 1 行动点 |
| T | 选择战场目标时 | 打开或关闭键盘选格模式 |
| WASD／方向键 | 键盘选格模式 | 移动目标光标 |
| Enter／空格 | 键盘选格模式 | 确认当前目标 |
| Ctrl＋Enter | 主角回合且没有待处理结算 | 结束回合并清空剩余行动点 |
| Home | 查看战场时 | 把镜头移回主角 |
| Esc | 任何战斗界面 | 取消当前操作，或关闭最上层可以关闭的界面 |
| R | 战斗背包中选中物品时 | 旋转物品 |
| F | 靠近可搜索物时 | 搜索容器；已有物品露出时拿取当前物品 |
| Ctrl＋F | 战斗背包 | 把输入焦点移到物品搜索框 |
| 数字键 1–4 | 战斗背包中选中战术道具时 | 放入对应战术栏位 |

Ctrl＋Enter 用于结束回合，Home 用于把战斗镜头移回主角。学院地图复位键另行确定；肉鸽模式不设置重开战斗快捷键。
...
教学按玩家当前做到的事情触发，不按固定秒数播放。一次只显示一条；已完成的不会重复弹出。
...
教学出现时暂停战斗，只允许当前教学要求的操作。Ctrl＋Enter 用于结束回合，Home 用于把战斗镜头移回主角。学院地图复位键另行确定；肉鸽模式不设置重开战斗快捷键。
