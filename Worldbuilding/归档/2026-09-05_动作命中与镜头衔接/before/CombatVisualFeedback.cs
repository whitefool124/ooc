...
namespace OCC.Combat.Presentation
{
    ...
    public sealed class CombatVisualFeedback : MonoBehaviour, IResolvedCombatFeedbackSink
    {
        ...
        private void Update()
        {
            ...
            { ClearVfx(); movementPlayback.Clear(); return; }
            ...
            {
                if (!unit.IsAlive) movementPlayback.Cancel(unit.Id);
        ...
        private void PlayDeliveryVfx(GridPosition source, GridPosition target, string effect)
        {
            ...
            vfxPlayback.ReplaceAbility(new[] { new CombatVfxCue(target, effect, 0f,
                FireVfxSequence.StageDuration, source, effect == "fire_projectile") }, Time.unscaledTime);
            RefreshVfx(Time.unscaledTime);
        }
