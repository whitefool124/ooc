...
namespace OCC.Combat.Presentation
{
    public readonly struct CombatMovementPose
    {
        ...
        public CombatMovementPose(Vector2 position, Facing facing) { Position = position; Facing = facing; }
    ...
    public sealed class CombatMovementPlayback
    {
        ...
        private sealed class Track
        {
            ...
            public CombatMovementPose Sample(float now, out int next)
            {
                ...
                for (next = 1; next < Points.Length; next++)
                {
                    ...
                    if (distance < segment)
                    {
                        Facing facing = delta.x > .001f ? Facing.East : delta.x < -.001f ? Facing.West :
                            delta.y > 0f ? Facing.North : Facing.South;
                        return new CombatMovementPose(Points[next - 1] + delta * (distance / segment), facing);
                    }
                    distance -= segment;
                }
                return new CombatMovementPose(Points[Points.Length - 1], FinalFacing);
            }
        }

        public bool Play(string unitId, GridPosition source, GridPosition destination, Facing finalFacing,
            IReadOnlyList<GridPosition> path, float now)
        {
            ...
            tracks[unitId] = new Track { Points = points.ToArray(), Length = length, StartedAt = now,
                ...
                Destination = destination, FinalFacing = finalFacing };
            return true;
        }

        public CombatMovementPose Sample(string unitId, GridPosition resolvedPosition, Facing resolvedFacing, float now)
        {
            ...
            return new CombatMovementPose(new Vector2(resolvedPosition.X, resolvedPosition.Y), resolvedFacing);
        }
