...
- A／B／C 中，战斗1的 A+B 已冻结并实现为水与灯藤；“雨后灯庭”的 12×9 固定地图、2 名敌人、出身三件套、确定性 AI、三条验收路线与最低可用说明已接入。首版及返修 v2 水面／灯藤均已被用户否决并降为 `CONCEPT` 追溯件；方格适配 v3 已通过机器门禁、用户暂时批准、稳定 GUID、Importer 与真实 B1 运行时复核，两项均晋级 `FORMAL`。战斗2、战斗3、精英、奖励、台词和其余具体机制仍未冻结。
...
- 底部右侧：互动、战术栏、结束回合。
- 顶部：背包、重开、离开。
- 术式栏名称在卡片内最多显示两行，费用区和 2px 细框不得被文字侵入；完整效果通过悬停说明查看。
- 术式编组槽固定显示名称与“行动、魔力、冷却”摘要；右侧详情按消耗、目标、效果连续排布，不使用无法容纳 24px 正文的矮厚框。
- 术式悬停说明固定按“当前状态、消耗、循环、目标、效果”排列，目标和规则必须转换为玩家语言，不显示内部枚举或规则 ID。
- 带九宫格厚边框的面板，其直接文字与内边至少保留 12px 安全区；高度不足以容纳安全区和正文时改用 2px 细框或无框分隔，不允许文字进入边宽。
- 右侧英雄状态首行只展开一个状态，其余状态显示为 `+N`，不得遮挡行动点和资源条。
- 指令不可用时保留按钮，并显示原因。
...
**缚环寻迹兽：** 12 生命、速度 11、导能撕咬为 1 AP／射程 1／4 物理伤害；缚环扑咬为 1 AP／1 魔力／冷却 1／射程 1／2 物理伤害并束缚 1 回合。可见主角时，若相邻且缚环扑咬可用则优先扑咬，否则沿固定最短路接近并撕咬。主角进入灯藤导致失去视线时，它沿主角最后进入藤丛的方向前往对应入口；抵达后完整停留一个自身回合嗅探；仍未重获视线则沿藤丛外围按顺时针格序搜索。其搜索目标、同距离路径优先级与格序必须写入遭遇数据。界面不显示最后目击点、搜索路线或灰色足迹；只通过移动、朝向、停顿与嗅探动画表达。寻迹兽没有其他涉水增益。

**高年级陪练生·火矢：** 16 生命、速度 9。火矢为 1 AP／2 魔力／冷却 1／射程 5／5 火焰伤害并施加燃烧 2 回合。焚藤开线为 1 AP／1 魔力／冷却 0／射程 5，只能烧毁预设关键灯藤，单个自身回合最多使用一次，且不会伤及藤内主角。可见主角且火矢射程合法时优先火矢；否则依次焚毁 F3、F4、F5；三格均已烧毁时，沿固定路线移动至下一北侧射击点。所有射击点、同距离路径优先级与烧藤目标写入遭遇数据，禁止运行时随机选择。

**公开信息：** 开战显示两名敌人、任务、地形、灯藤／水悬停说明和敌人下一行动意图。寻迹兽失去视线后的确定性搜索规则可通过悬停说明查看，但不额外标示内部目标格或路线。

**验证路线：** 灯藤斩首：从西南侧进入藤丛，令寻迹兽停在入口嗅探，从另一侧出现集火火矢；湿地强攻：主动走水沟，在燃烧后以水灭火换取继续前压；掩体清犬：依靠 C8 轻掩体先处理寻迹兽，再走干路逼近火矢。三条均须以当前开局装备、四个基础术式和三件套可完成；第三条更慢、战损更高，但不得成为唯一安全路线。
...
- 水面与灯藤首版候选因分别误读为金属框板与圆形徽章，于 2026-09-04 被用户否决并降为 `CONCEPT` 追溯件。水面返修 v2 虽通过机器门禁，但用户随后明确判定水面与灯藤素材均不合格，故水面 v2 与藤屏式灯藤 v2 均只保留为 `CONCEPT`。v3 已按方形俯视地图重新生成：水面使用“完整单块石板＋无框薄水膜＋断裂反光”，灯藤使用“北高、侧围、南低的方格中空树篱＋中央人物站位空腔”；两项均建立独立原料、manifest、32px 交付与单位／重复铺设接触图，通过机器门禁、用户暂时批准、稳定 GUID、Importer 与 B1 真实运行时接触，现为 `FORMAL`。

完成证据：主工作树与 Unity `Application.dataPath` 一致；Unity 重编译无编译错误；全量 EditMode 735/735 通过。2026-09-04 已在 Play Mode 从启动页、新游戏确认、固定出身、学院地图进入 B1；随后以真实战场双击先移动至 `(3,5)`，再进入 `(4,4)` 灯藤，主角由 3 AP 降至 1 AP，底层确认 `IsLampVine=True`，运行时截图同时覆盖 C7-G7 连续方形浅水与 E1-F5 中空灯藤。测试后退出 Play Mode、恢复原 2895 字节存档，场景未保存。首版与返修 v2 保持追溯件；v3 正式资产晋级 2/2。
...
1. 若继续做战术验收，按三条固定路线分别完成整场通关并归档战斗日志；当前已完成启动→固定出身→地图→B1，以及角色实际进入灯藤的真实交互验证。
2. 冻结战斗2的正式地图、B+C 特色地块、敌人、公开意图与至少 3 条理想路径；冻结前不实现、不生成候选图。
3. 冻结战斗1奖励三选一，并确保战斗2能对刚获得的构筑提供即时反馈；战斗3、精英和精英后奖励继续后置。
4. 依次冻结战斗3、精英、三个事件、法宝、工坊／医务室合法对象与商店内容，再进入对应阶段 B 数据接入。
