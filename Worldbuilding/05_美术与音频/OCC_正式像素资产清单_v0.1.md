...
| 资产 | 尺寸 | 状态 | 审查结论 |
| --- | --- | --- | --- |
| `hero` 单位静帧原料 | 64x64（规范化审查副本） | `QA_PENDING` | 自动 QA 将单张独立原料规范化为边界 `[22,12,42,58]`，对齐 `X=32` / `Y=58`；但目标尺寸下细节混杂且未经手工像素清理，不能导入或标为 `FORMAL`。证据见 `像素资产原料/V2-04/QA/occ_unit_hero_v01_review.md`。 |
| 其余 4 单位、5 图标、5 中继站对象 | - | `MISSING` | 本地工作台上游返回 `429 Too Many Requests`；未以旧拼板、脚本图形或未经 QA 的替代品填充。 |
