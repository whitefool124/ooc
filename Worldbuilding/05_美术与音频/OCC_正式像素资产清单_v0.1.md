...
## V2-13 战利品箱开启静态反馈（2026-07-25）

| 资产 | 原料/QA 路径 | 尺寸与状态 | 审查结论 |
| --- | --- | --- | --- |
| `loot_crate_open` | `像素资产原料/V2-13/Relay32/` 与 `QA/loot_crate_open/` | `32×32`，原料库 `FORMAL` | 独立原料经硬 alpha、16 色、4x QA 与 JSON 报告复核；仅用于 `LootContainer.IsLooted` 的静态反馈。 |

| Unity 资产 | Unity 路径 | 运行时用途 | 导入/运行时复核 |
| --- | --- | --- | --- |
| `loot_crate_open` | `Assets/Game/Resources/Art/FormalRelay32/loot_crate_open.png` | 已搜刮战利品箱的开启状态 | `32×32`、Sprite、Point、Clamp、无 mipmap；Play Mode 确认关闭/开启两种贴图均可加载并按 `IsLooted` 分支。 |
...
- 本地多帧生成一致性暂不作为正式运行时依赖；动画条目保持 `DEFERRED`，除非后续由产品重新确认。
- 正式运行时默认使用已通过 QA 的独立静帧；待机使用整像素微位移，受击使用短时整像素抖动，必要时使用颜色/亮度脉冲。
- Aseprite 已安装于 `E:\SteamLibrary\steamapps\common\Aseprite\Aseprite.exe`，仅用于后续人工清理、导出与 QA，不改变“独立单图原料优先”的流程。
