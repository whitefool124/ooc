...
- 下一候选为 `SAVE-INTEGRITY-01`：为 `map9` 增加语义不变量、坏档备份与写保护；启动前先把任务合同补入本文件，再进入实现。
...
- **唯一当前主任务：** 当前无实施中主任务；下一候选为 `SAVE-INTEGRITY-01`，启动前先补充具体任务合同。本轮不并行扩充地区、敌人、法宝或术式。
- **当前路线：** 先修复影响资源经济和正式 HUD 的双快捷栏，再按 `SAVE-INTEGRITY-01 → STARTER-BUILD-01 → SOURCE-OF-TRUTH-01` 处理存档、初始武器与源文件一致性。
...
- **状态说明：** 下方 V1/V2-01/V2-15/V2-22 既有 `IN PROGRESS` 与“保持 V2-15 为唯一主任务”等文字是暂停前的历史快照；`CORE-INTEGRITY-01` 已完成，下一候选为 `SAVE-INTEGRITY-01`，历史状态不得重新解释为并行任务。
...
1. `CORE-INTEGRITY-01`：统一实例背包与八格快捷栏，消除跨战斗免费消耗品。
2. `SAVE-INTEGRITY-01`：增加 `map9` 语义不变量验证、坏档备份和写保护测试。
